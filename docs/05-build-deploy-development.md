...
Стенд поднимает [owlab](https://github.com/owfeed/owlab) из `owlab.yaml` в корне
...
**Релизные пакеты собираются не так.** Их делает owfeed из подготовленного дерева
(`./tools/stage.sh && owfeed build` — оба формата, секунды, без тулчейна; docs/13). Ниже — путь
через SDK: он существует, чтобы тема оставалась собираемой Makefile'ом и luci.mk у человека,
который про owfeed не слышал, и чтобы это можно было проверить.
