 ...
 * the DOM — issue #56) ----
 *
 * luci-app-bandix's render() calls document.body.appendChild() unconditionally for 7 tooltip/modal
 * nodes, on EVERY visit — it never checks whether they already exist and never removes the previous
 * copies. An SPA nav away used to leave every one of them behind — unstyled once
 * scopeToCurrentPage() darkens bandix's own <style> on the swap, sitting under the footer — and a SPA
 * return to bandix duplicated every id, a fresh render() appending a second copy over what was still
 * there. Reproduced on owrt2512 and owrt2410.
 *
 * Nothing here deletes them: whether an app's own append runs once, at module eval (the CSS shape
 * Rule 1 already covers — ACE, HexEditor), or on every render() the way bandix's does, sweeping it
 * on the way out is not something a generic classifier can do safely. A node an app's own code still
 * expects to find is the same one-way break Rule 1 describes for a sheet imported once, and nothing
 * here can tell which shape a given app's code takes. The only correct exit is the one
 * documentPoisoned() already takes for an invasive sheet: decline the SPA nav and let the browser
 * reload, which starts the next page with an empty <body>.
 *
 * The recorder — the first <script> in header.ut's <body>, before this template renders a single
 * chrome element — fills window.__fsBodyAdds with every element a script has ever parked directly on
 * <body> through the instance's own appendChild/insertBefore/replaceChild/append/prepend/
 * insertAdjacentElement/insertAdjacentHTML. This file only reads that list. */
