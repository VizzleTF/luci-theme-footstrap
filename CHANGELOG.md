...
- **Content that arrives below the reader no longer moves them — a regression this release introduced and caught.** 0f298ef made the growth witness in `observeContent()` climb to the floored box a refill sits in (`closest()`), so a refill one level inside a box finally counted as growth — but nothing asked WHERE that box was. For a box that grew below the reader, the reader's reference correctly did not move, and `lateDrift()`'s blind-witness branch read that as a blind witness and wrote the whole growth. Measured deterministically with a probe that grows a floored box below the viewport by 120 px: the offset moved +120 and the reader -120 on firefox and chromium, `/admin/network/dhcp` and Overview, with the engine's own anchoring ON — an ordinary browser — and in the DECLINES state, 8 of 12 cells; the call stack named `settle()` in `lateDrift()`. The same probe on the fs-fit.js from 21c0417, before that change, moved nothing in all 12. The witness now counts growth only from a box that starts above the reader's reference: a box that begins at or below it cannot move it. The probe then reads 0 px in all 12 cells. The sweep could never see this — every case grew content above the reader — and gains BELOW for it (next entry). `lateDrift()`'s decision trail also gains four markers (`frame`, `wait-frame`, `wait-idle-moving`/`wait-idle-intent`, `settle`); they are what showed the late corrections on firefox and webkit waiting for a frame the page did not produce.
