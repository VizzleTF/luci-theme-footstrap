...
- **The Appearance sliders look the same on 23.05 as on every later release.** Where `ui.RangeSlider` is missing the theme builds its own, and that copy rendered a unit suffix from `calcunits` alone; the real widget renders it only when a `calculate` function produced a value, and no axis here passes one. Measured on the stands: five sliders with `px`/`%` on 23.05, none on 25.12 — so a router upgrading off 23.05 watched them disappear. The polyfill now renders what the real widget renders, and the dead `unit` option is gone with it.
