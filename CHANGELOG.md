...
- **A table no longer shreds a one-token column into a vertical ribbon instead of carding.** `overflow-wrap: anywhere` gives such a cell a min-content of one character, so the table always "fits" and the overflow measurement never fires — it reports the truth and it is useless. Reported from a hardware router at 700–790px of window: the DHCPv4 lease table cards there, because its `nowrap` IPv4/MAC/IAID columns give it a floor and it really does overflow, while the DHCPv6 table beside it stays a table and breaks the DUID THROUGH — measured on that router at 5 lines with 674px of room and 7 lines at 654px, against one line at 1160px; the reporter's screenshot shows three characters to a line, twelve lines tall, next to a v4 table already in cards. `fitTables()` now also cards a table with a single-token cell past **two** lines, alongside the first-column tower test it has had since issue #7. **Whitespace is the whole discriminator**, and it separates the two cases cleanly on real pages: every multi-line cell on that router was dumped and classified at 1440 / 1000 / 860px, and the only tokens reaching three lines are the two DHCPv6 DUIDs — everything else that goes tall has spaces in it and is wrapping exactly as intended (Processes' command lines, 11 lines at 860px; the assoclist's `229.4 Mbit/s, 20 MHz, HE-MCS 9` rates; the `Access Point "vaka" (phy1-ap0)` badges), and none of them cards. A census over all 196 menu pages of the stand at three widths finds **0** token cells past two lines, so nothing there changes at all. Cost on the worst page there is — Processes at 860px, 16 rows tall enough to hold a ribbon — is **0.19 ms** per pass, gated row-first so a short row is one `clientHeight` read.
