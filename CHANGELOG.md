...
- **The session works as a crew: an Opus lead thread that delegates, and `developer`, `tester`, `security` and `researcher` subagents in `.claude/agents/`.** The lead reads no screenshots and runs no gate — the heaviest session on record put 484 Bash results and 20 screenshots through one context against 5 delegations, and that is what kept filling it. One writer, sequential; a read-only tester on a different model than the writer, covering T0 through both owlab stands and, on the maintainer's word for the task, the hardware router; at most two fix rounds, and a repeated blocking finding is a stall that goes to the human. `tools/bg-wait.sh` is the waiter every detached run is paired with. In `.claude/settings.json` the owlab commands moved from `ask` to `allow` and `dev-sync.sh` and `ssh` into `ask`; the branch guard and the comment/review fences are gone — publishing anything without an explicit instruction is a rule in `CLAUDE.md` now, not a matcher. `CLAUDE.md` dropped from 206 lines to 168 by pointing at `docs/` where it used to repeat it: the comment rules are in `docs/conventions.md`, the probe rig and the hardware proof in `docs/development.md`, the protocol in `docs/crew.md`.

- **The crew's own tooling is fenced where the permission matcher cannot reach.** `Bash(tools/bg.sh:*)` is allow-listed and the matcher reads the first word of a command, so `tools/bg.sh sh -c '… ssh …'` — the exact form every T2 gate uses — carried `ssh`, `dev-sync.sh` and `git push` straight past the `ask` rules that guard the hardware router and publishing. The wrapper now refuses `ssh`, `scp`, `sftp`, `rsync`, `dev-sync.sh`, `git push|commit|tag` and `gh pr|release|issue`, scanning words rather than substrings so `sshd` and a path ending in `-ssh` do not trip it, and with globbing off so a run carrying a literal `*` is read as text; all ten real T2 invocations in `docs/development.md` still pass. `tools/bg.sh` also writes `<run-id>.pid` from the inner shell, and `tools/bg-wait.sh` stops on three things instead of one — the status file, that pid being gone, or a cap of 7200 s (`tools/bg-wait.sh <run-id> [interval] [max]`) — because a run killed by SIGKILL or by the machine sleeping never writes a status, and the waiter used to wait for that file forever and then wake a finished session with it. `tester`, `security` and `researcher` carry `disallowedTools: Edit, Write, NotebookEdit`, which makes read-only a fence rather than a sentence in a prompt; `git stash|clean|restore|reset --hard|checkout -- ` and `git add -A|--all|.` moved to `ask`, the loop owning the index between rounds and `/upstream-pr` needing all of them; and `permissions.additionalDirectories` finally registers `../tmp`, without which the three `Read/Write/Edit(../tmp/**)` rules never applied and every card write prompted. `precommit-gate.sh` reads the `-a` flag only from the part of the command before the first quote — `git commit -m "add -a flag"` used to compare against HEAD and deny a commit over dirty files it did not hold — and stands aside with a warning when `jq` is missing instead of erroring before every bash call. `measure-run.mjs` now also counts `<session>/subagents/*.jsonl`: the 2026-09-03 trial reads 152k characters in the lead against 1.54M across 17 subagent transcripts, which is the half of the trade the lead's own number cannot show.

### Fixed

- **A checkbox with a tooltip no longer draws a second, dead toggle switch.** ui.js's `ui.Checkbox.render()` puts a SECOND label inside `.cbi-checkbox` whenever `form.Flag.tooltip` is set — `label.cbi-tooltip-container`, holding the hint icon, next to the switch label the theme means to style — and the switch rules matched both: reported against Network → Firewall → Zone Settings' "IPv4 Masquerading" (luci-app-firewall's `masq`, forum topic 251930, post 118), where luci-theme-bootstrap shows a checkbox plus a ⚠️ and footstrap showed a toggle plus a second, unresponsive grey pill. The pill/knob/hit-target rules in `theme/60-inputs.css` and the forced-colors border in `theme/95-a11y-media.css` now key on `.cbi-checkbox > label[for]`, the switch label only — `for` is what tells the two apart, `render()` giving it to the switch label and never to the tooltip one, and it is also the cheapest discriminator: the sibling form of the same fix costs 100 B more of shipped CSS; the tooltip label falls back to a plain icon glyph with an 8px gap (`--fs-space-2`), 0.5px off the switch's own top edge — measured on the gallery's new fixture, both labels were 40x22 and flush before this, one is now 16x21 with a real gap after.

- **`postinst`/`postrm` no longer call `rpcd reload`, closing the race that can permanently drop an unrelated rpcd plugin.** Measured on owrt2512: `rpcd reload` is `exec_self()`, a full re-exec that re-`dlopen()`s every plugin `.so`, and a file absent or mid-write at that instant is dropped for good (`mv file.so /tmp; rpcd reload` loses `file` from `ubus list`; restoring the file alone does not bring it back, only a further reload/restart does). Losing `file` this way is what makes System → Backup/Flash Firmware's "Reset to defaults" row silently vanish (`view/system/flash.js` reads `L.resolveDefault(fs.trimmed(...), '')`, so the `-32000 Object not found` answer reads as "no overlay" — forum topic 251930, posts 97/109). The theme ships no rpcd plugin of its own — only an `acl.d/*.json`, which rpcd re-reads **at login**, not only at start (measured: no reload needed for a restored ACL file to take effect on the next login) — and root's stock `read='*' write='*'` login makes the theme's ACL group moot for a root session regardless (measured `true` even against an invented object). The one case a reload used to help — a session already logged in on a *restricted* rpcd login before the install — is now handled by ending a session only when a fresh login would actually gain the scope: denied `uci`/`footstrap`/`write` right now, **and** its own username's rpcd login lists the theme's ACL group; every root session, every session that already has the scope, and every denied session whose login was never given the group in the first place (a second admin account, a monitoring script's login), is left running. The same call is gone from `dev-sync.sh`'s post-copy step for the same reason — it copies files onto a live router the same way, the theme still registers no rpcd object, and a developer's root ssh session already carries the stock blanket grant — leaving only the LuCI cache removal there. `docs/package.md`.
