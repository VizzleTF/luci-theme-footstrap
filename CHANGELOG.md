...
- **`docs/development.md` now names two stand traps that read as theme faults.**
...
- **The crew's own tooling is fenced where the permission matcher cannot reach.** `Bash(tools/bg.sh:*)` was allow-listed, but the matcher reads only a command's first word — so `tools/bg.sh sh -c '… ssh …'`, the exact form every T2 gate uses, carried `ssh`, `dev-sync.sh` and `git push` straight past the `ask` rules guarding the hardware router and publishing.
  - `tools/bg.sh` now refuses `ssh`, `scp`, `sftp`, `rsync`, `dev-sync.sh`, `git push|commit|tag` and `gh pr|release|issue`, scanning whole words with globbing off — so `sshd`, a `-ssh`-suffixed path, or a literal `*` all read as text; all ten real T2 invocations in `docs/development.md` still pass.
