...
- **postinst runs the theme's uci-defaults itself again, as it did through 0.14.13.** The previous cleanup left registration to `default_postinst` alone; CI's `motion` slice, which measures Overview on the first load right after install, went from passing every run to failing 3 of 4 attempts on the two commits that carried that change. The call is idempotent, so the second pass changes nothing on a router where `default_postinst` already ran it.
