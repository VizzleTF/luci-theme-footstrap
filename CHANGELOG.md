...
- **The theme runs on OpenWrt 23.05, where it used to lose its own settings page and its stylesheet.** 23.05 already has the ucode dispatcher and ucode theme templates, so the server half needed nothing: every `.ut` compiles with that release's own `ucode`, the dispatcher exposes the same globals, and rpcd carries the file methods the uploads use. Two things did not survive. `ui.RangeSlider` arrived in 24.10, so on 23.05 the constructor was `undefined` and the WHOLE Appearance tab failed to build — no theme settings at all, over one missing widget out of five; it now falls back to a local class that reproduces the upstream widget's own markup and its `setUpdateEvents`/`setChangeEvents` pair, and is reached only when the real one is absent. And `luci.mk` on 23.05 defines `Build/Prepare` without the `$(call Build/Prepare/$(LUCI_NAME))` that arrived in 24.10 — the hook where this package generates `cascade.css`, mangles its tokens, strips the templates and stamps `FS_VERSION` — so the package would have installed and served a 404 for its own stylesheet; the same copy loop is restated after the include with the call put back. Measured on a 23.05.6 router: chrome, menu, login page, SPA navigation, dark mode and card-stacked tables all render with no console errors, and the Appearance tab builds with 24 rows whose sliders drive their axes live.
