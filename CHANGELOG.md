...
- **The log filter bar keeps each filter on one line instead of breaking a label away from its control.** `tools/views.js` builds it as a run of inline text — `<label>Не</label><input type=checkbox><label>объект:</label><select>…` with nothing around a pair — so inline flow breaks wherever the line runs out, and on a phone that is between a label and the control it names. Reported from a 390px screen: the second `Не` sat alone at the end of one line while the `уровень:` it inverts opened the next one without it, and `Макс. строк:` broke INSIDE the label, leaving `Макс.` above `строк:` and the field they belong to. The two rows are flex containers now, so a wrap can only fall between items and a label is one unbreakable item; the fields shrink instead of forcing the break (`min-width: 0`, which undoes the generic field box for this row alone), and the max-rows box is capped at 12ch so it stops reading as a second search field. Below `fs-view` 560px the first row — a fixed alternation of two filters, four items each — becomes a four-column grid, which puts exactly one filter per line. The second row is deliberately left to wrapping: it holds two filters of different shapes, and the same four tracks let its max-rows box define column 2 and push the text filter off the card (measured). Keyed on the ROW'S SHAPE rather than on the page or on the controls' ids: `data-page` differs between the two logs and a third-party LogreadBox has neither, and the ids are no better — only the System Log is built by `tools/views.js`, while Status → Kernel Log is a view of its own whose four rows carry `logFromTime`, `logToTime`, `invertLogRangeTime`, `invertSeverity` and `invertAscendingSort`, of which an id list written against LogreadBox catches none (`logFacilitySelect` appears there only as a `for=`, on a label whose control is a time field). Both views wrap their rows in `#content_syslog`, and a filter row is exactly one that opens with a `<label>` — the scroll-to-end button's row and the log itself do not — so the Kernel Log's three extra rows now get the same treatment instead of keeping the inline flow. Generalising them raised their weight past the two rules they override, and inside a layer specificity is read before source order, so both had silently stopped applying: `#logMaxRows` alone is (1,0,0) against the field rule's (1,0,3), and the narrow-viewport grid keyed on `div:has(> #logFacilitySelect)` is (1,0,1) against the row rule's (1,0,2). Measured against that revision: the max-rows box stretched like the free-text filter, 510px against 98px at 1280, and at 390px the four-column grid never fired though its `grid-template-columns` computed. Neither could be re-anchored under `#content_syslog` — two ids in one selector is [2,0,x], past the [1,7,0] ceiling `css-metrics` holds — so both key on shape too: the row's `input[type="number"]`, the only one in either log's filter bar, at (1,1,3), and the row with two sibling selects, which is the same row `#logFacilitySelect` names and the one the four-column template is sized for, at (1,0,4). The fields carry `min-width: 0` **and** `max-width: 100%`, which answer different layouts: the first lets a flex item shrink past its content, the second clamps a grid item to its track — a `<select>` keeps the theme's own 210px field width, which a `minmax(0, 1fr)` track does not override, and both selects hung 3px past the view at 360px until it did (caught by the full-page sweep, not by the report).
