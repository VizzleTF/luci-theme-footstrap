...
- **`fonts/set-font.sh` removes the `--mono-bold-file` CLI option: the theme assigns `<strong>` the sans face, so a bold mono face was never read by anything, and the flag is now refused like any other unknown option instead of silently accepted.** The `add_slot`/`SLOTS`/newline-`IFS` staging loop is also replaced with three direct `[ -n "$X_FILE" ] && stage …` lines — `stage()` was already only reached for a slot whose file variable was non-empty, so checking that variable directly reproduces the loop's behaviour for every option that remains. 360 to 336 lines; `sh -n fonts/set-font.sh` passes.

- **Shipped JS carries less duplicate logic.** `fs-axes.js` exports each axis as the object itself (`tint: TINT`, …) instead of a `currentX`/`applyX` alias pair per axis; the idempotent attribute-writer and the Enter/Space activation handler, each copied into three modules, move into one `fs-widgets.js` export apiece; `partials/footer.ut`, with one includer, is inlined into `footer.ut` and the partial file is gone. No behaviour change — `npm run smoke`'s export and ordering contracts hold unmodified — and the flash-served bundle drops from 94.5 KB to 93.0 KB.

- **`build-css.sh` strips comments and squeezes whitespace in one string-aware awk pass instead of two chained ones, and `mangle-tokens.sh`'s O(n²) awk selection sort is replaced by a `count name` list piped to `sort -k1,1nr -k2,2`.** `cascade.css` and the mangled token map build byte-identical to the two-pass version; the fixed 80 KB `FS_CSS_FLOOR` guard is gone, replaced by the brace-balance-plus-rule-count check the build already ran, proven to still catch a truncated sheet before the guard was dropped.

- **CI and `tools/ci-local.sh` run the same `npm run check:fast`/`check:mid`/`check:slow` scripts instead of restating each gate step by hand, and two standalone checks fold into `stylelint`.** The `!important` allowlist (`tools/bang-ok.mjs`) is now `declaration-no-important` plus its existing override list, and the `[data-title]`-safe selector check (`tools/css-i18n.mjs`) is now a `selector-disallowed-list` regex; both replacements were proven equivalent by planting a violation the old tool caught and confirming stylelint still catches it.

- **11 `tools/*.mjs` scripts parse CLI flags with `node:util`'s `parseArgs` instead of hand-rolled `arg()`/`argv.includes()`, `tools/lib/stands.mjs` gains one shared `owlabRouters()`/`toStand()` pair instead of two copies, and `tools/css-metrics.mjs` walks the sheet with `css-tree` instead of the `@projectwallace/css-analyzer` dependency, now removed from `package.json`.** `css-metrics`'s importants/emptyRules/specificity counts are unchanged from the dependency's own numbers on the current tree; the orphaned `tools/readme-shots.py`, called from nowhere, is deleted.

- **`styles/**`'s comments are cut to the invariant, its measured number and one pointer, per `docs/conventions.md`'s 40% cap.** History and rationale prose that was only ever going to be read once moved into `docs/css.md`, `docs/design-system.md`, `docs/chrome.md` and `docs/third-party-apps.md`. Aggregate comment share across every `styles/**/*.css` file drops from roughly 78% to 37.7% of bytes (98,011 of 260,062 bytes); `cascade.css` builds byte-identical, checked per file against a before/after build.

- **The 11 `tools/*.mjs` scripts that already used `parseArgs` drop the `const arg = (name, dflt) => FLAGS[name] ?? dflt` indirection in favour of `default:` on the option itself, `.claude/tooling/lib.mjs` reads `PORTS`/`login` from `tools/lib/stands.mjs` instead of a hand-kept port map, `tools/playground/build.mjs` inlines `lib.mjs`'s `stableStringify`/`ubusKey`/`stripBase`/`isSafeReturn` into the built page via `fn.toString()` so `replay.js` stops carrying hand-copies of them, `tools/lib/css.mjs` and `gallery.mjs` stop re-exporting `ROOT` (5 callers now import it from `tools/lib/root.mjs` directly, matching `tools/lib/page-modules.mjs` and `tests/lib/luci-module.mjs`, fixed the same way), and the `.github/workflows/build.yml` boot step duplicated across jobs `live` and `anchors` (identical but for one env var only `live` set) is now one `tools/ci-boot.sh`.** No behaviour change: `npm test` (154 tests) and `npm run smoke` pass unmodified.

- **Dead extension seams and their pre-24.10 fallbacks are cut from the shipped chrome.** The plugin/search-source hooks — `loadPlugins()` and the `fs_plugins` whitelist, `header.ut`'s/`head.ut`'s `window.__fsPlugins` emission, `fs-search.js`'s `globalSources`/`pool`/`window.__fsSearchSources`/`window.__fsSearchGen`/`onTake`/`keyOf`, and the exported `remember`/`nodeForSegs` — go, along with `pageOf()`'s now-unreachable `#`-section split: `git grep` across this repo and every sibling checkout (`../luci`, `../luci-app-footstrap-files`, `../luci-app-footstrap-updater`, `../luci-app-gitbackup`, `../owfeed`, `../podkop_autoupdater`) found zero readers of any of the three globals outside the theme's own copies. `fs-appearance.js`'s `parseColor()` drops its string-parse fallback for "no 2D context" (canvas 2D is universal on 24.10+) and `colorControl()` drops its single-caller `current`/`opts` defaulting; `fs-router.js` drops the `typeof L.Poll.active`/`!L.Poll` fallback and two `typeof dom.content` guards (`L.Poll.active`/`L.dom.content` are unconditional on the supported floor); `fs-select.js` drops its `typeof inst.setFocus` fallback (`ui.Dropdown.setFocus` ditto); `fs-axes.js`'s `snapshotAxes` stays module-private, its only reader being internal; `menu-footstrap.js`'s single-caller `wireDismiss()` is inlined. No behaviour change: `npm test` (153 of 154 — the one case driving the removed `!L.Poll` branch is gone with it), `axes`, `chrome-fence`, `page-modules` and `css-orphans` all pass unmodified.

- **A second round on the same shipped JS and templates cuts stale documentation, a duplicate loop and null-guards for elements the chrome always emits.** `fs-prefs.js` drops three comment blocks documenting axes that already moved to `fs-axes.js` (`colorAxis`/`surfaceAxis`/`propAxis`, Save-to-router), the unused `require rpc` pragma, and two branches whose result already matched the fallthrough right beneath them. `fs-assets.js`'s three "verified NOT exploitable" SVG-sanitizer findings move to `docs/architecture.md` behind a one-line pointer each (sanitizer code itself unchanged), and `_readText`'s FileReader wrapper is replaced by `file.text()` at its one caller, `.catch()` keeping the same localised error message. `fs-select.js` merges its two identical table-role loops into one and calls the theme's own `widgets.syncAttr` instead of a private `setRole` copying the same body, drops `inRoots`'s `roots` parameter (every call already passed `liveRoots()`), and drops a `sb.querySelector('ul')` fallback openwrt-24.10's `ui.js` never needs — `ul.dropdown` and `sb[open]` are set and cleared together in the same function. `fs-chrome.js`, `menu-footstrap.js` and `menu-footstrap-common.js` drop null-guards on `#tabmenu`/`#modemenu`/`#topmenu`/`#fs-rail-toggle`/`#indicators`/`#fs-search-btn`: all six are emitted by `header.ut`/`notices.ut`/`search.ut` whenever these modules load at all (`!blank_page`), and `menu-footstrap-common.js`'s module-list comment now names what `fs-widgets.js` actually exports. `fs-overview.js` drops a dangling comment, a `data-page` check `watch()`'s only caller had already made, and the `_observedRoot` re-attach machinery — `#maincontent`, unlike `#view`, is never replaced by an SPA swap. `fs-sheets.js`'s `currentKey()` drops its `location.pathname` fallback: `L.env.dispatchpath` is stamped by luci-base's own `header.ut` before any theme script runs, on every page including the login form. `head.ut` folds six `(v != '' && n>=a && n<=b) ? n : -1` range clamps and three `/^[a-f0-9]{6,64}$/` token checks into two ucode helpers, and `sysauth.ut` replaces a 4-line manual-focus script with `autofocus` on `#luci_password`. No behaviour change: `npm test` (153), `axes`, `chrome-fence`, `page-modules`, `i18n` and `smoke` all pass unmodified, and `computed-diff` reports 0 of 905 elements x 62 properties changed; roughly 9.5 KB removed across the ten files touched.

- **A third CSS round: five light-palette values shared once instead of repeated per palette, four small structural merges, and two build-script self-checks a later step already covers.** `styles/03-palettes.css`'s `--fs-panel-base` and the four `--fs-on-*` inks are `#fff` in every LIGHT palette block; hicontrast/bootstrap/2020/forum's light blocks now inherit them from the default `:root, :root[data-palette="footstrap"]` block instead of restating them, `docs/design-system.md` "Adding a palette" documented accordingly — proved with a scratch `getComputedStyle` probe over the 5 tokens across all 5 palettes x 2 modes, worktree vs HEAD, 0 mismatches. `theme/20-shell.css`'s, `theme/55-buttons.css`'s, `theme/60-inputs.css`'s and `theme/65-dropdown.css`'s `background-image: none` right after a `background:` shorthand in the same rule is dropped (7 sites): the shorthand already resets it. `theme/20-shell.css`'s three adjacent `@media (max-width: 767px)` blocks merge into one. `--fs-bar-ctl-bg`, `pre`'s local `--border-color` and `--fs-port-min` — each read only where it is declared — are inlined at their one call site. `theme/70-modal.css`'s `.modal` box-shadow rule merges into `theme/45-misc.css`'s existing `.modal` rule (same layer). `build-css.sh`'s two error-path `rm -f "$body"` calls drop (the EXIT trap already covers them); the success-path one stays — measured: dropping it too left `$OUT.tmp.<pid>.layer` behind on every clean build, since the trap is disabled before that point. `mangle-tokens.sh` drops its sort line-count self-check and a `[ -d "$d" ]` check `--rewrite` had already run one loop earlier; the collision check stays. `npm run computed-diff --full` (10 palette x mode points, 2 viewports): 0 differences; `./tools/stage.sh && owfeed build`: 6 packages, mangled map 0 collisions.

- **A second round on workspace tools, tests and CI cuts a dead browser-support classification, a redundant CI step, a thin wrapper and its own test, and four hand-rolled argv parsers.** `tools/css-floor.mjs` drops the `view-transition`/`view-transition-old`/`view-transition-new` pseudo-element and `aspect-ratio` property classifications — `grep -rn` found neither anywhere under `styles/`, so they could never be reached from the built sheet — and `--update` now writes the baseline from what the current build actually uses (`known = [...seen]`) instead of a set union, which never shrinks; `tools/baselines/css-features.json` loses two stale entries (`backdrop-filter`, `word-wrap`) no longer emitted by any rule. `.github/workflows/build.yml`'s `check` job drops its standalone `build-css.sh /tmp/cascade.css` step — its own comment already said "nothing downstream reads this copy", and the same script's brace-balance/short-file check already runs inside `check:mid`'s `css-metrics`/`css-floor`/`css-dup`, in the `lint` job every push and PR also runs; `tools/ci-local.sh`, which otherwise restates that step so a local run matches the workflow, drops its three mentions of it (the job-description line, the `build-css` step call, and the `--list` matrix row) to match. `tools/playground/lib.mjs`'s `rewriteHostname` wrapper (one line, calling `rewriteLiteral` with the same three arguments) is gone; `build.mjs` calls `rewriteLiteral` directly and `tests/playground.test.mjs` drops the now-redundant `rewriteHostname` test, since `rewriteLiteral`'s own covers the same shape. `tools/computed-diff.mjs` and `tools/pseudo-loc.mjs` parse their CLI flags with `node:util`'s `parseArgs` instead of a hand-rolled `ARGV.indexOf`/`.includes()` pair, same flags and defaults. `tools/smoke.mjs` and `tests/lib/luci-module.mjs` import `RESOURCES` from `tools/lib/page-modules.mjs` instead of each restating `join(ROOT, 'luci-theme-footstrap/htdocs/luci-static/resources')`. `package.json`'s `lint:js` is one `eslint` invocation over both `htdocs` and `ucode` instead of two. Four tests (`interval-pause`, `session-expiry`, `router-contract`, `poll-status`) drop a `window.L = L`/`win.L = luci` assignment ahead of `loadModule('fs-router', …)`, which already sets `win.L = win.L || luci` from the same `L` it is handed — every caller passes a freshly built `window`, so the fallback and the manual assignment always agreed. `SHAPE_PROBE`/`PROBE_DEADLINE_MS` (`tools/lib/page-shapes.mjs`), `extractScripts`/`assertDataOnly` (`tools/lib/ut-scripts.mjs`) and `makeRule` (`tools/lib/eslint-no-unsanitized.mjs`) drop `export`: each is read only inside its own file. `.github/workflows/pages.yml`'s push-trigger paths name `tools/lib/gallery.mjs`, which `tools/devkit-build.mjs` does not import; replaced with the four files it does reach — `tools/lib/css.mjs`, `tools/lib/tokens.mjs`, `tools/lib/root.mjs` and, through `css.mjs`'s `buildCss()`, `luci-theme-footstrap/build-css.sh` — so an edit to any of them now triggers the portal rebuild it should have all along. The `FS_SHAPES` page-shape cache (`tools/lib/page-shapes.mjs`, set by `.github/workflows/build.yml`'s `live` job) is gone: `parity`/`audit`/`motion` are three matrix jobs on three separate runners, and `docs/ci.md` already said the split "gave that saving back" — at most one of the three calls `classify()` per stand, so the cache's own write, at the end of a runner's one and only walk, had nothing left in the same process to read it back; classification itself is unchanged, only the on-disk write/read and the env var around it are gone. `docs/ci.md`'s "three jobs, not one" section is reworded to match: the 421s/303s figures are named as measurements of the combined job the split replaced, not a claim about the current one. Two stale comments are corrected: `tools/build-usign.sh`'s header named two CI jobs as callers, of which the release job (now owfeed's own reusable workflow) has not called it since the SDK build left CI — `build-apk.sh`'s local, non-CI SDK path is the one remaining caller; `tools/playground/lib.mjs`'s `isSafeReturn` comment said it was duplicated verbatim in `replay.js`, which `build.mjs` has inlined by `fn.toString()` since the round above. `npm test` (152), `lint:js`, `css-floor` all pass unmodified.

- **Stale counts, a dead docs entry and a dead permissions changelog fixed across the docs.** `docs/conventions.md` and `docs/development.md` stop citing "four routers"/"four palettes" (`owlab.yaml` is now the count of record) and fix "eight" to the real ten palette x mode combinations (5 palettes x 2 modes, `styles/03-palettes.css`); `docs/README.md` drops the nonexistent `design/` doc entry; `.github/ISSUE_TEMPLATE/bug_report.yml`'s palette dropdown lists the real five — Footstrap/Hi-Contrast/Bootstrap/OpenWrt 2020/Forum (`fs-appearance.js`) — instead of three including a nonexistent "Rvht" entry, and points at the Appearance page (`/admin/system/footstrap`) instead of a popover; `docs/crew.md`'s permissions table restates current `.claude/settings.json` state instead of an "ask -> allow" changelog, and the retired `tester-b` trial narrative is one line; `update-po.sh`'s two rename/bug-history comments trim to their invariant, with the history moved to `docs/package.md`; `luci-theme-footstrap/README.md` and `owlab.yaml`'s own header point at the `routers:` list instead of restating "four", and `owlab.yaml`'s `extra_packages` anchor comment drops its stale "other three RELEASE routers" count (5 now alias it, after the imm removal below) rather than carry a number that drifts every time a router is added or removed.

### Removed

- **`.claude/tooling/lib.mjs` drops the unused `PAGES` and `navAndCheck` exports, and `owlab.yaml` drops the two ImmortalWrt routers from the default `owlab up` boot.** `grep -n` over `.claude/tooling/*.mjs` found no importer of either export; `docs/development.md`'s probe-rig paragraph is trimmed to match. `imm2512`/`imm2410` are not a gate target (`tools/lib/stands.mjs`) and answer no behaviour claim (the "second instances" comment in `owlab.yaml`), and owlab 0.6.1 (`owlab up --help`) carries no opt-in/profile/autostart key to mark them optional in place — `docs/development.md`'s new "ImmortalWrt stands (on demand)" note carries both router blocks verbatim and the `owlab up imm2512 imm2410` command that boots them when a question is actually about the ImmortalWrt brand.
...
- **Dead CSS rules and unread tokens.** Duplicate `.cbi-page-actions` button colour rules a later cascade layer already re-sets, the flex-era `::after` clearfix and its `vertical-align` rule, the floated `.secondary-action` and its RTL mirror, a `pre{border-radius}` restating `code, pre`'s own, the `--fs-warn-soft`/`--fs-warn-line`/`-hi` tokens and their twins, and 2 page files (`pages/60-assoclist.css`, `pages/90-processes.css`) that emitted no rules at all. `node tools/css-dup.mjs`/`fs-orphans.mjs` confirm zero remaining readers of the deleted tokens; `npm run computed-diff` shows zero computed-style change on every affected selector.

- **The Makefile's `FOOTSTRAP_PREMIN` and `FOOTSTRAP_VERSION` knobs, and the postinst's re-run of `uci-defaults/30_luci-theme-footstrap`.** `git grep` found no setter for either knob anywhere in the tree. The uci-defaults re-run was measured dead on both package managers — `owrt2512b` (apk) and `owrt2410b` (opkg), fresh install and upgrade, 4 of 4 runs — the file was already gone and the theme already registered by the time the postinst's own guard ran, since the package manager's own `default_postinst` runs first.

- **`styles/02-tokens.css`'s `:root[data-density="large"]` drops `--fs-density-space: 1`.** Redundant, not dead vocabulary: it restated the bare `:root` block's own default of `1` at the same cascade point, so the computed value is unchanged. `npm run computed-diff` reports 0 of 905 elements x 62 properties changed, both appearance points, both viewports.

- **20 orphaned `.claude/tooling/*.mjs` probes, 2 stale design drafts (`docs/design/cats_final3.svg`, `OpenWrt-Status-Redesign.reference.html`) and 3 unused `owlab.yaml` stands (`owrt2512c`, `owrt2410c`, `owrtsnapc`).** `git grep` on each deleted basename found no reference outside this changelog; the `-b`/`-d` twins are unaffected.

- **22 more orphaned `.claude/tooling/*.mjs` probes, the 2 stale `bench/results-container-{2410,2512}.json` benchmark snapshots, and 7 CLI flags no caller ever passed, now constants.** Probes: `spa1`-`spa5`, `align`, `apply`, `phone`, `pattern-script`, `ro2`, `rtl`, `scroll`, `heap`, `snap`, `leak`, `fwprobe`, `fwprobe-chrome`, `splify2`, `design-ab`, `recon-ab`, `parity`, `overflow` — `git grep` on each found no reference outside this changelog. `lang.mjs` is the one probe kept: `tools/live-audit.mjs`'s own `--lang` header comment still points at it. The hardware `bench/results-25.12.json` stays. Flags: `fit-quiet --slack`, `floor-contract --slack`/`--wait`, `table-tick --slack`, `live-audit --arrive`, `playground/capture --stand`, `scroll-anchor --density`. `tools/lib/export-contract.mjs` and `tests/lib/fit-page.mjs` fold into their one caller each (`tools/smoke.mjs`, `tests/fit-state.test.mjs`) — `git grep` on each name found no second reader.
...
- **`strip-assets.sh` strips `logo.svg`'s XML comment and `manifest.json`'s indentation the same way on every awk, and refuses to ship a no-op silently.** Both awk programs relied on `BEGIN { RS = "\0" }` to slurp a whole file into one record, a gawk/mawk-only extension; neither macOS's bundled awk (one true awk, 20200816) nor busybox awk 1.38 has that case, and both fall back to `strlen(RS) == 0`, paragraph mode, which split `logo.svg` on its one blank line and left its `<!-- -->` comment in place — `./tools/stage.sh` printed `strip-assets: 2 file(s)` and shipped 1338 B instead of 593 B, and CI only ever worked because its runner's `/usr/bin/awk` is gawk. Both loops now accumulate lines in the main `awk` block and process the buffer in `END`, no `RS` tricks, and a post-strip guard refuses (exit 1) if `<!--` or a newline survives. Verified byte-identical output across macOS `/usr/bin/awk`, `busybox awk` 1.38 and Debian `gawk` 5.2.1: `logo.svg` 593 B, `manifest.json` 309 B, on all three. `docs/development.md`, "Building a package locally".

- **A user-picked hex accent/good/warn/danger colour now wins its ink (`--fs-on-*`) in every palette's dark mode, not just the default one.** `styles/03-palettes.css`'s `[data-accent="hex"][data-accent]` block and its three siblings were (0,3,0) by the repeated attribute, tying a *named* palette's own dark block (`[data-palette=…][data-darkmode="true"]`, also (0,3,0)) — 20 losing rows (5 named palettes x 4 axes), measured with a Playwright probe over all 5 palettes x 2 modes x hex/no-hex, worktree vs HEAD. Fixed by repeating the attribute a second time (`[data-accent="hex"][data-accent][data-accent]`, (0,4,0)): the hex rule now outranks that dark block on specificity, not source order — `css-metrics`' max-specificity budget stays [1,7,0], no `!important`. `npm run computed-diff -- --full` (905 elements x 62 properties x 10 appearance points x 2 viewports): 0 differences, since the gallery carries no hex pick.
- **postinst runs the theme's uci-defaults itself again, as it did through 0.14.13.** The previous cleanup left registration to `default_postinst` alone. The call is idempotent, so the second pass changes nothing on a router where `default_postinst` already ran it.

- **`tools/scroll-jank.mjs` waits for the Overview to actually paint before arming its measurement, instead of a fixed 2600ms clock.** The gate's only flaky cell, `chromium owrt2512 @768 side /admin/status/overview`, failed 4 of 16 PR runs and 0 of ~80 push runs, always with the same numbers: `moved 1980/0 anchor 4044px polled-in 30 shift 0.1013`. PR runs boot only `owrt2512` (`tools/ci-boot.sh`), so this cell starts ~15-17s after container start, ~20s sooner than a push run's later stands; `fs-overview.js` keeps every section hidden until `network.flushCache()`'s five RPCs answer, and on a router that just booted, one answers late enough that the fixed wait expired first — ARM recorded `scrollable 0` with no tables rendered, then the page finished painting mid-scroll and read as a 4044px jump the "nothing scrolled" guard (`scrollable > 120`) never catches. Not the postinst change above: `446a09f` carries that fix and still failed this cell twice. The fixed wait is now a bounded (20s) `page.waitForFunction` for a rendered `.cbi-section`/`.table` plus `scrollable > 0`, then a short 400ms settle; a page still unpainted at the deadline is reported as its own failure, `"page not painted, run proves nothing"`, instead of a silent pass. The wait is now exported (`lib/stands.mjs`'s `waitForPaint`): `spa-parity.mjs`'s Back case had the identical fixed wait before the same Overview goto (CI run 36184976382, `owrt2512` parked at 0px, misread as "no room to scroll") and uses the same helper now.

- **A view that appends its own nodes straight to `<body>` and never removes them now hands the next SPA navigation over to a full load, instead of leaving them unstyled under the next page's footer.** `luci-app-bandix`'s `render()` calls `document.body.appendChild()` unconditionally for 7 tooltip/modal nodes on every visit; `dom.content()` only replaces `#view`'s children and `sweepAround()` only reaches `#view`'s siblings, so nothing ever cleared them, growing the document by ~1600px on the Overview (issue #56). An inline recorder, the first `<script>` in `header.ut`'s `<body>`, wraps `document.body`'s own `appendChild`/`insertBefore`/`replaceChild`/`append`/`prepend`/`insertAdjacentElement`/`insertAdjacentHTML` as instance properties (never `Node.prototype`, so an extension's isolated-world content script is never recorded); `fs-router.js`'s `strayBodyNode()` classifies each recorded node (skipping the chrome, LuCI's own `#modal_overlay`/`.cbi-tooltip`/hidden download anchor, and script/style/link nodes) and `bodyLittered()` counts what remains attached, `navigate()` declining exactly where `documentPoisoned()` already does for an invasive sheet — Rule 2 (`docs/third-party-apps.md`) extended from CSS to the DOM. Nothing is swept: a node an app's own code still expects to find is the same one-way break as a sheet imported once. `fs-chrome.js`'s and `fs-appearance.js`'s two permanent measurement probes gain `fs-*` ids for the same reason the chrome has them, so they read as ours and not as litter. `tests/body-litter.test.mjs` (12 cases) drives the classifier, the counter and `navigate()`'s decline directly; `tools/smoke.mjs` runs the recorder itself against a real `document.body`.

- **A bare `.cbi-progressbar` now reserves its floated value's own line in flow by default, instead of `margin: 0` and a bet that whatever wraps it happens to leave room above.** A key/value `.table` row off the overview page — `luci-mod-status`'s `20_memory.js` markup, reused verbatim by `luci-mod-dashboard`'s Resources tab (forum thread 253559 post 54) — carries no context rule for it, so the value spilled 5-6px above its row and printed across the divider belonging to the row above. Default is now `margin-block-start: calc(var(--fs-type-xs) * var(--fs-leading))`, upstream `luci-theme-bootstrap`'s `margin: 1.4em 0 4px` model, the same expression `.fs-stacked` and the overview `<=560px` rule already used. The three contexts that deliberately place the value BESIDE the bar or on a neighbour's line opt out with `margin-block-start: 0`: `.cbi-value-field`/data-table meter columns (pinned in `@mirror meter/beside`), overview's meter rows at desktop, and the package-manager disk bar at both desktop and phone (page layer wins unconditionally, so one declaration covers both widths). New `docs/gallery.html` fixture reproduces the reported shape; `npm run computed-diff` shows the reserve appearing only on bare meters, zero change on every beside context. The same gap hit `ui.js`'s upload progress dialog (Backup/Flash): its value text overlapped the dialog title by 10px, now sits clear — measured live on both stands.

### Security

- **`build-css.sh` and `mangle-tokens.sh` close 3 LOW findings from the security review of their own simplification above.** The `/*!` license-banner scan matches the marker at any position on a line, not only at its start, so a banner glued to preceding code still survives the strip; `mangle-tokens.sh`'s `sort` call now checks its own exit status and that the sorted line count still equals the pre-sort count, exiting non-zero on either mismatch instead of silently shipping a wrong or truncated token map; and no file path is interpolated into an awk-built shell command string any more — both scripts pass the path through `ENVIRON` or a plain shell expansion, so a path containing `"` can no longer break out of the quoting. Proven by planting each fault: a mid-line banner still survives, a forced `sort` failure now exits the build non-zero, and mangled output is unchanged from the pre-hardening worktree build.
