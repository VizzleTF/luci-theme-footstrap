...
- **The anchor sweep runs in about half the wall clock, with the same cells.** It was serial across engines and stands while the stands are independent containers and there are now two of each OpenWrt line, so a sweep now runs engines concurrently and splits a stand's cells with its twin. Measured on an identical axis (webkit, two stands with twins, 36 runs): 390 s to 216 s, 1.81x, from pairing and session reuse alone. Coverage is untouched — the reporting is the same code moved verbatim — because this week showed three times what a missing cell costs: `390 top` outside the default axis hid 41 px, chromium without `--full` hid a regression on the commonest engine, and one refill instead of three hid a 47-60 px drift a reader met every five seconds. `--quick` gives the local loop a smaller run that says loudly what it skipped; CI keeps the full one.

- **`docs/development.md`'s stand traps gain six more from this session, all about running npm and long gates on a Windows checkout.** An install run from WSL instead of Windows writes `node_modules/.bin/` as POSIX symlinks Windows cannot execute, so every `npm run <gate>` from Git Bash and the git `pre-push` hook then die with `'eslint' is not recognized …`, reading exactly like a red gate rather than a bad install — installed from Windows instead, npm writes a sh wrapper WSL can run too, invoked as `node "C:\Program Files\nodejs\node_modules\npm\bin\npm-cli.js" ci --ignore-scripts` from Git Bash (`cmd.exe /c npm ci` from Git Bash silently does nothing; PowerShell refuses `npm` outright, see below). A fresh Windows-side install then needs `chmod +x node_modules/.bin/*` from WSL before WSL can run any of it — `/mnt/c`'s `fmask=0177` mount option leaves every file npm just wrote world-unexecutable, and the fix does not survive the next `npm ci`, which writes new files under the same mount default again. PowerShell is not a fallback for the symlink trap: `npm.ps1 cannot be loaded because running scripts is disabled on this system` is its own execution policy, unrelated and unaffected by which side installed. And `${PIPESTATUS[0]}` reads back empty in this WSL bash, the same way `$?` is already unreliable there — a piped `npm run check | tee log; echo "CHECK_EXIT=${PIPESTATUS[0]}"` printed `CHECK_EXIT=` while the run had actually failed on the size budget, with nothing but the gate's own output to say so. And `tools/bg.sh`, started from inside a `wsl.exe -e bash -c …` call, dies with that call: the log file is created with `bg.sh`'s own header and never grows, and neither `.status` nor `.pid` ever appears — the inner shell is killed before it writes one — so `tools/bg-wait.sh` has no pid to test, cannot see the death, and waits out its full two-hour cap (`WAIT-RC=2, exit: timeout (no status after 7200s)` on a run that died at second zero), while from the caller's side the start is indistinguishable from a gate that ran and exited 0. The detach is real inside WSL; what does not survive is the WSL session, torn down with the interop call the moment its own command returns. A long gate from a Windows host goes in a background Bash-tool command instead. And `"no owlab router is running, so nothing was checked"` from every live gate at once, with all eight stands up, means the shell could not find `owlab` — a Go binary in `~/go/bin` that the login profile puts on `PATH`, and a `wsl.exe -e bash -lc` login shell can die part-way through that profile on something unrelated (measured: a stale `deno/env` path from another project, printed as a bare `No such file or directory`), leaving `PATH` half-built; `owlab status` listing routers as `running` tells the two apart, and `wsl.exe -e bash -c` with `export PATH=$HOME/go/bin:$PATH` skips the profile entirely. Six gates saying "nothing was checked" instead of exiting 0 is `21a8502` and `916d4d5` working as built.

- **The `developer` and `tester` roles get turn caps a whole task fits inside — 50 to 200 and 80 to 140.** Both were set when a task meant one file and one gate. This session's anchoring work runs a role across a multi-file card, three stands and three engines, and both caps were reached mid-task: the role returns whatever it had, which reads like a finished answer and is not one. The caps are still caps — a role that has not converged by 200 turns has a card problem, not a budget one.

- **The four crew roles answer in caveman full, the mode the lead runs in.** A subagent inherits no SessionStart hook, so `/caveman full` reached the lead and stopped there: the four role prompts kept returning full prose into a thread that was itself compressed, and the lead quotes those blocks verbatim. Each role file carries its own "Voice" section now — compress the prose around the data, keep commands, flags, paths, numbers, quoted errors and every negation exact, and never add a word to sound caveman. What a role writes to disk is untouched by this: code, comments, a changelog line and a handoff stay normal English prose. `docs/crew.md` and the delegation contract in `CLAUDE.md` say the same, so a hand-written delegation carries it too.
...
- **The theme stops rewriting floors that have not changed — 725 clears and 625 writes per 25 s of polling become 70 and 70.** `holdFloor()` cleared and rewrote `min-height` on every floored box under `#view` on every pass, and 610 of those 625 writes put back the identical value; only 15 changed anything. Measured on owrt2512, Overview, 25 s of real ticks. This is a cost fix and nothing more: a probe that suppressed the churn entirely showed the engine still correcting in 6-18 ms and the reader moving 0 px, so the rewriting was never suppressing anyone's anchoring — it was simply work a router does not need to do while it routes packets. What the floors themselves buy is real and unchanged: with them muted the clamp reaches 426-443 px. `tools/floor-contract.mjs` gains the case that guards the risk this creates — a floor that fails to come down because a skip decided it was unchanged — and it was seen red on a deliberately over-eager skip (11 findings) before the real one was accepted.

- **Three live gates watch the reader instead of the fix that was made for them.** `tools/fit-quiet.mjs` measured the sidebar's own height — that is, the pin `fitChrome()` installs — so growth anywhere else above the reader read as 0 px; it now measures the content's own position in the document, and going back to the pre-`49a1ff5` one-way pin makes it report the 107 px and 41 px its header always claimed. `tools/table-tick.mjs` asserted a CSS rule (`.fs-dt:not(.fs-fitted)` has height 0) rather than the 612 px jump it exists for; it now compares the footer's document position before, during and after a tick, and reads +417 px on the isolated defect. `tools/live-audit.mjs` sampled geometry 180 ms inside the window the theme defers on purpose, which is how it reported a false break at 568 px; geometry is now sampled both immediately and past a settle, every other check only past it. And `scroll-anchor`'s new REPEAT case measured nothing at all in 274 cells — it searched for its own section instead of reusing the one SWAP had settled on; at full parity (39 of 39) it immediately found 31 real findings, the repeated-refill drift fixed in this release.

- **Eight gates could not fail, and now can.** An audit of every gate for one question — is it green because it looked, or because it did not — found `tools/fs-orphans.mjs` passing on an empty `styles/` (deleting the entire stylesheet was a pass), `tools/axes.mjs` silently dropping a check when an axis is renamed with a capital (`data-raiL`), `tools/css-i18n.mjs` reporting "0 files" as success, `tools/a11y-gallery.mjs` calling a blank page "clean across all 24 combinations", and `tools/computed-diff.mjs` — the gate this project proves a CSS change with — unable to go red at all in the form `npm run computed-diff` invokes it. Each now fails on an empty or skipped measurement, and each was demonstrated red on the very defect it was introduced for: the 1.69:1 white-on-green contrast, a lost `text-align`, a drifted `@mirror` pair, a `return //` eating the rest of a minified file. `mirror`, `css-dup` and `jsmin-verify` got the same floors.

- **Back returns the reader to where they were, on a phone as well as a desktop.** The live gate measured them dropped from 2475, 2684, 4747 and 5110 px to 0 on the Overview -> package-manager route, on two stands and both widths, and a probe had seen the same at 390/top (1577 px to 0) — while the identical Back at 1440/side restored correctly. Three separate causes, all in `restoreScroll()`: a cancellation that watched BOTH scrollers, so a scroll on the axis the restore did not target killed it; `fit.forgetRest()` gated on a forward navigation, so a Back left the previous page's anchoring reference standing (the comment above the call already claimed otherwise); and the browser's own traversal restore landing BEFORE `commitStage()`, on a document still about 900 px tall, so the engine clamped it to 0 and that clamp read as the reader taking over. A scroll landing exactly at the scroller's own current ceiling, while that ceiling is still short of the target, is the engine settling — a reader cannot scroll past a height that does not exist yet. `tools/spa-parity.mjs` gains the case that catches it: nothing in the suite tested the history at all, which is how this survived.

- **`lateDrift()` no longer reads a blind reference as "the engine put it back" — the correction that recovers a poll refill the engine declined to anchor now has a witness that cannot be blind to the growth it measures.** `fs-fit.js`'s residual check compares the reader's remembered reference element against itself, and that element is whatever `anchorRef()` hit-tested at the fold on the last still page — not guaranteed to sit below the container a given tick actually refilled, so a reference unaffected by THIS growth honestly reads 0px of drift and the code reads that zero as "corrected." CI failed the same cell across three SHAs (1e88310, 7ff9e56, 76b3c7a): `webkit owrtsnap @1440 side compact, engine-anchoring on, overview` — a section refilled and the page never came back, still 120px off after the 900ms window; `drift < 1px` fired 13 of 13 refills, `anchorRef()` having landed on a plain `DIV` at top -362 in compact density (top -33 on `DIV.network-status-table` in normal) that the growth never reached; `_lateMisses` never advanced because incrementing needs a write and `lateDrift()` never wrote one — the raw scroll offset stayed at `+0` for the whole window. `observeContent()`'s `MutationObserver` callback now reads `records` (previously discarded) for the first `childList` record whose target already wears `data-fs-floor` — the mark `holdFloor()` writes at the height a container had before the tick — and passes the height it actually gained, in pixels, to `lateDrift()` as a second witness: a container's own `offsetHeight` cannot answer "nothing grew" falsely the way a borrowed element's position can. The other four refusals are untouched. `tools/scroll-anchor.mjs`'s "never came back" finding now prints `swap.writes` too — the "corrected late" finding beside it already did, which is exactly why three CI runs of the same finding could not say whether the theme wrote nothing at all or wrote somewhere the mark did not see. Verified on a rebuilt `owrtsnap`: the failing cell reads `swap moved 0px [offset +120] corrected 16ms`; `normal`/`large` densities stay green on the same cell, chromium/firefox stay at 0px on every default-axis `overview` cell, and `fit-quiet` plus `SWAP`'s own `tick` case are unmoved. **This shipped with a claim this entry no longer makes — "a false witness only ever refuses, never writes" — that was reasoned, not measured, and the very next CI run measured it false: 21 findings, both webkit and firefox, all on `/admin/network/dhcp @390`, every density, both layouts, both stands, every one printing `writes: [{"how":"window.scrollTo",…}]` beside a `swap.clamped` the gate itself already reported, the overshoot matching that clamp in every one — task detector, same day.** Instrumented directly against the failing shape: the 32-36-row leases table `SWAP` empties and refills grows its floor box by 128-132px against a 120px pad (table-row rounding at 390 wide, not the pad), while `compensated` — how much the OFFSET already moved since the reference, `seen - ref.at` — reads exactly 120px: the engine had already carried out the whole correction, and the witness's own rounding (8-12.25px, the exact overshoot of every finding) was the only thing the formula still read as outstanding. `lateDrift()` now tells the two cases apart by what the OFFSET did: `compensated` exactly zero is the blind case above, unchanged, and the whole growth is still the correction; anything else means the engine already responded, and the residual is this witness's own noise, not a number to write — it counts as a miss (`_lateMisses`, the same bookkeeping `_engineTrusted` above already keeps) and `lateDrift()` returns without writing, leaving the correction to `anchorFor()`/`scheduleAnchor()` once `LATE_MISS_LIMIT` trips, which does not share this failure. All 21 CI-failing cells re-measured through the real gate against the fix: `swap moved 0px`, engine alone, no write logged — zero findings, 24 runs on webkit and 24 on firefox; the overview cell above is unaffected (`compensated` there matches `grow` to within rounding, so neither write fires, and it already passes on its own). Costs 8 B minified over `tools/size-budget.mjs`'s `coldJs` limit (58,900 B; the checkout stood at 57.5 KB, comfortably under, before this correction) even after folding the branch into the leanest form measured (three variants tried against the real gate — nesting the miss-count inside the existing `if (Math.abs(g) >= 1)` cost 10 B over, duplicating `Math.abs(g)` in a combined condition cost 26 B over) — reported rather than raised, per the budget's own rule; `npm run size` (`size-budget.mjs`) is red pending that call. `docs/anchoring.md` carries the mechanism and the numbers under "A witness that cannot be blind". **This fix shipped a claim of its own that was not measured either: that any non-zero gap between `compensated` and `grow` is a miss, `>= 1`.** Two ticks (10s, `LATE_MISS_LIMIT` is 2) after it landed, `_engineTrusted` went false on `/admin/network/dhcp @390` while the engine was still anchoring that page correctly — the 8-12.25px table-row rounding this same fix already knew about, now counted as a miss instead of ignored, took over the fast `anchorFor()`/`scheduleAnchor()` path **while the engine's own anchoring stayed fully on**, precisely what `ENGINE_ANCHORS`'s own comment warns against ("running both is not a safety net: two corrections throw the page the other way") — task missrule, `../tmp/task-anchor-audit/inventory.md` §2.2, found the same day: "the most important unmeasured consequence in the tree." `SWAP` performs one refill and closes, so nothing before this could see the switch trip on the SECOND miss — all 48 green sub-runs above were first-miss runs. `tools/scroll-anchor.mjs` gains `REPEAT`: `REPEAT_TIMES` (3) of `dom.content()`'s empty-then-refill cycle on the same section back to back, reading the reader's position after each one and `fs-fit.engineTrusted()` (new, unmarked export, same reason `restAt()` is) at the end, so "the reader never moved" and "the switch stayed trusting" are two separate assertions instead of one inferred from the other. `lateDrift()`'s gap check now compares against `LATE_ROUND_TOLERANCE` (16px — 3.75px/30% of head-room over the largest of the three measured gaps, 12.25px, the same margin-over-the-worst-measured-cluster shape `LATE_MS` already uses) instead of a bare `1`: the three measured gaps (8/12/12.25px) no longer count as a miss at all, a gap past 16px still does. Verified against the formula directly (the three measured gaps, the new tolerance's own edge, and a 60px genuine-partial-failure control: only the first three flip from miss to no-miss) and live, the real gate code against the real fix, route-interception, never synced to the shared stands (`../tmp/task-missrule/probe3.mjs`): three refills on a correctly-anchoring `owrt2410`/chromium/`overview` cell read `moved: 0, 0, 0`, trust `true` throughout; three refills on a DHCP table this stand's engine genuinely never anchors at all read `moved: -132, -131, -180px`, trust `true -> false` on the second, exactly as built — the switch still trips where the decline is real. Costs 20 B minified over `tools/size-budget.mjs`'s `coldJs` limit (58,908 B measured before this correction, 58,928 B after; the existing 58,950 B limit needs no raise, 22 B of head-room left, not 42). `docs/anchoring.md` carries the mechanism, the classification table and the live numbers under "A miss must mean the engine did not do the job". **`REPEAT` shipped measuring nothing, anywhere: a full three-engine sweep over the same three routers reported 274 of 274 cells `nothing above the reader big enough to collapse`, so a switch that only flips on the SECOND miss was still invisible to everything, this case included.** Its own body/mark search ran the identical selector and park SWAP's does, but a SECOND time, after SWAP's own two `swap()` passes had already woken `holdFloor()` — and a fresh search there can land on a smaller or absorbed candidate than the one SWAP just proved reaches the scroller (measured: an independent search picked `table.table.cbi-section-table`, 1145px, where SWAP's own search had found a sibling that DID reach it). A second, separate defect hid behind the first: `grewDoc` was read AFTER all three of `REPEAT`'s own pads had already been removed, net zero on purpose, so even a correctly-picked candidate always reported "the growth never reached the scroller." `REPEAT` now reuses `window.__fsRepeatSection` — the section SWAP already validated, stashed the moment SWAP settles on it — instead of searching again, and reads `grewDoc` on its first refill while that pad is still standing, matching SWAP's own timing. Re-run on the same three routers, all three engines, default axes: `REPEAT` now measures all 39 of the 39 cells `SWAP` itself measured in the same run (100% parity, up from 0) — and immediately found 31 real findings, unforced, on `chromium`, `firefox` AND `webkit` alike: the reader holds through the first refill of `/admin/status/overview` and `/admin/network/dhcp` but drifts 47-60px on the second or third back-to-back refill of the SAME section, `corrected: never` — the exact shape `../tmp/task-anchor-audit/inventory.md` §2.1 named "the most important unmeasured consequence in the tree" and could only describe, never catch. Confirmed as a real signal rather than a fixture quirk: an isolated cell that reads clean unpatched (`owrt2410`/chromium/`/admin/status/overview @390 side`, `moved: 0, 0, 0`, trust `true` throughout) reads `trustedAfter: false` with the reader still not moving once an instrumented copy of `fs-fit.js` (route-intercepted, never synced) is made to answer `engineTrusted()` falsely from its second call on — proving the case fails as readily as it passes. This fault in `fs-fit.js` is unfixed and out of this change's scope; `SWAP`, `HOLD`, `QUIET` and `TICK` report the same numbers as before on every cell (0 findings between them, `mid-flick surprises 0` on all 156 runs).

- **`fs-select`'s "does this table have room" check no longer answers off a viewport width up to 256px stale for the first 220-400ms after a resize.** `fs-chrome.js`'s `contentWidth()` read `_shellOuter`, a cache written only by `measureShell()`, which only `fitShell()` calls — and `fitChrome()` steps aside for the whole `SCROLL_IDLE` window (400ms, `fs-fit.js`) whenever `fit.scrolling()` answers yes, which a resize starts the same way a flick does. CI's `live-audit` failed on exactly that shape (`owrt2410 /admin/status/vnstat2/config | 568 | geometry | fs-content (-178)`, run 34364446910): `-178` is not a layout number, it is `568 - 390`, the gap between two adjacent widths in the gate's own list, and the same shape held at every step (`-70` at 320→390, `-256` at 1024→1440) — `../tmp/task-vnstat/probe2.mjs` read `model=358 real=536 scrolling=true` at 220ms and `off=0` only once resampled past 620ms. `contentWidth()` now compares `document.documentElement.clientWidth` against `_shellOuter` on every call and re-measures when they differ, instead of only when `_shellOuter` was still unset — the one layout read this was already conditional on for a caller arriving before any fitter had run, extended to a caller arriving mid-resize too. Costs 39B minified over `tools/size-budget.mjs`'s `coldJs` limit (58,650B, 29B of head-room left). `docs/chrome.md` carries the numbers under `data-narrow`, and `docs/development.md`'s "The stand's own traps" gains the shape of this finding, so a future `geometry|fs-content` offset matching the gate's own `WIDTHS` gap is read as this race, not a fresh regression.

- **Content hidden IN PLACE now gives its floor back — on the theme's own Appearance panel and on any stock LuCI form that uses `depends()`.** `fs-fit.js`'s `observeContent()` carried three `MutationObserver`s, none watching what actually hides most content: `fs-appearance.js`'s `foldable()` OPENS a disclosure by mutating nodes (`refreshColours()`) but CLOSES it by writing `hidden`/`aria-expanded` only, and `form.js`'s `setActive()` — what every `depends()` calls — hides a row by toggling the CLASS `hidden` on the `[data-field]` element, neither of which woke `run()`, so the `min-height` `holdFloor()` took while the section was open never came off. Measured on `/admin/system/footstrap`: "Colours" grew the section 731px → 1485px and STAYED at 1485px closed, 754px of empty ground still there 21s later on a page that never polls ("Background" the same shape, 55px held); stock LuCI hits it too, wider — System → System, Time Synchronization, unticking "Enable NTP client" left 308px of floor over a 50px bare section, 258px unchanged 10s later. `_moTabs` now also watches `hidden`/`aria-expanded` unconditionally — cheap, nothing rewrites them on a poll tick — and `class` narrowed to a mutated element carrying `data-field` (`r.target.dataset.field`, a property read with no forced layout, once per delivered record), rather than across the whole subtree, which the comment above `_moFlag` already forbids: the poll rewrites row classes every tick and an unfiltered watch would call `run()` — a forced layout — on every one of them. Both cases now read 0px held, immediately; `fit-quiet` and `tools/scroll-anchor.mjs`'s `tick` case are unchanged at 0px, since the new coverage answers to a real attribute change, not to a poll tick. `tools/floor-contract.mjs` gains the two triggers this needed — a disclosure opened then closed, a `depends()` row switched off — its own ACCURACY check caught the discrepancy outright once something exercised it. Costs 82 B minified over `tools/size-budget.mjs`'s `coldJs` limit (58,500 B, 45 B of head-room before this fix) — the attribute list and the `class` filter are both irreducible without dropping coverage for one of the two cases, so the limit is reported here rather than raised; `npm run size` is red pending that call. `docs/anchoring.md` carries the numbers as the seventh item on `holdFloor()`'s load-bearing list.

- **On the branch where the engine anchors, the theme no longer gives a misbehaving engine a third 420ms-late chance — and the gate that watches for it can now tell "never corrected" from "corrected late".** `fs-fit.js`'s mutation observer forked on `ENGINE_ANCHORS`, a check asked once at load of whether the platform supports scroll anchoring at all — blind to an engine that has the property but declines to keep the reference across a given container refill, which CI showed does happen. On that fork, `lateDrift()` (the cleanup path for an anchoring engine's own residual, not the primary corrector) was the ONLY net, and task latenet's four-way SWAP ablation measured what that costs when it is doing the work alone: `overflow-anchor: none` suppressed alone (engine off, theme still trusting `ENGINE_ANCHORS`) carries the correction at 419-420ms, against 7ms (chromium/firefox) to 32-36ms (webkit) when the theme's own fallback (`applyAnchor()` via `scheduleAnchor()`) runs instead — the base, untouched case corrects at 0ms on all three engines. `lateDrift()` now counts every residual it actually has to write back (`_lateMisses`); once two have been measured on a page (`LATE_MISS_LIMIT` — one is headroom for a single one-off, two is the count), `_engineTrusted` goes false and every later refill on that page takes the fast, non-engine path instead, no matter what `ENGINE_ANCHORS` still answers. The comment at the increment site claims only what is measured — "this engine did not keep the reference … N times on this page" — never a browser name; chromium and firefox measure 0 residuals on the pages this sweep covers, so the switch never trips for them. `tools/scroll-anchor.mjs`'s `SWAP` case gained the blindness that let a real regression here read as a pass in the first place: it read the mark once at a fixed 800ms delay, so a `--settle 300` read reported the full drift and the same cell at `--settle 500` reported 0px, with nothing to say whether the correction was fast, late, or never came — the CI failure that opened task latenet. `swap()` now samples the mark every frame for 900ms (comfortably past the measured 419-420ms) and records the first frame the drift falls back under tolerance: still outside tolerance at the end of the window is reported as "never came back"; corrected but past 200ms — picked to sit roughly midway between the measured 7-36ms and 419-420ms clusters, with well over 150ms of margin either side — is reported as "corrected late", a finding of its own even though the reader ends up in the right place, since 420ms of visible drift is what a maintainer reading a report calls a jump. Separately, `HOLD` used to report only a moved delta with no geometry: a `-505px` finding on `firefox owrt2512 @1440 top compact` cost a full round chasing a fault that never reproduced, because there was no way to tell "the correction failed" from "the mark ended at the viewport's own top" (`before.top` read 503 on every local run; `after.top≈0` was the second thing, not the first). `HOLD` now returns and prints `before.top`/`after.top` on every reading, plus the scroll-write log an `addInitScript` wrapper records for the whole context (`scrollTo`, `scrollBy`, the `scrollTop` setter, `Element.scrollTo`, borrowed from `../tmp/task-holdreg/hold-probe.mjs`), and a `HOLD` reading past tolerance is re-measured once on the same page before it may become a finding — one flake in a ~216-cell sweep must not fail a run on its own. `.github/workflows/build.yml`'s chromium sweep (the `motion` slice) ran with no `--full` on either a pull request or a push, so chromium's own `compact`/`large` densities went unmeasured on every run; it now splits `--full` on push/tag only, matching the split `anchors` already makes for firefox and webkit. `docs/anchoring.md` carries the numbers and the two gate outcomes; `ENGINE_MISANCHORS`/`data-fs-anchor-suppress` stay out of the tree — this is gated on a measured runtime residual, not a browser's identity.

- **A stale CI comment undercounted the anchor sweep's own axes.** `.github/workflows/build.yml` said a pull request crosses "12 cells per router against 48" and named "the two scrollers" — both true when the comment was written, neither true since `390 top` joined `1440 side` and `390 side` in the default axis (commit `accc451`, because the WebKit-only tick fault it targets measures 0px at `390 side` and 41px at `390 top`). Recomputed from `tools/scroll-anchor.mjs`'s own current `PAGES`/`SCROLLERS`/`DENSITIES` axes (3 pages × 3 or 4 scrollers × 1 or 3 densities × 2 anchor states, per router): a pull request now crosses 18 cells per router, a push or a tag 72. Comment only — no job, step or matrix changed.

- **`tools/scroll-anchor.mjs`'s `markAt()` can no longer land its mark on a `position: sticky` element, closing a latent gap in the gate rather than a live bug.** A sticky element's `rect.top` is pinned to its stuck offset for as long as it stays stuck, so `after.top - before.top` stops being a faithful proxy for the reader's place the moment the stick state does — or does not — change mid-measurement, the same failure shape the gate already guards against for `#view` and `.cbi-section-descr`. Found while chasing a webkit finding on `owrtsnap @1440 side compact` (run 34120078154) that would not reproduce in 7 follow-up attempts: the mark had landed on `th.th`, the sidebar's own sticky table header (`theme/30-tables.css`), where `owrt2512` gave `td.td` for the same page and point. It did not fire in any of those runs (`headerWasPinned=true, tableTop=-166`, moved 0px throughout), so the original finding needed no fix — but nothing in `markAt()` made that true on purpose. A synthetic sticky header proves the guard does something: `markAt()` without it read the sticky mark's `rect.top` as 0 on every growth from 0 to 120px, while a plain sibling at the same point moved with the page — a mark landing there would pass however badly the theme failed. Both copies of `markAt()` (`HOLD` and `SWAP`) now walk each candidate's ancestors up to `#view` and skip one with `position: sticky` anywhere in the chain. Re-run as a full webkit sweep (`--full --engines webkit`) across `owrt2512`, `owrt2410` and `owrtsnap` afterwards: 184 runs, 0 findings, exit 0 — the same shape the original run reported, unchanged.

- **`tools/scroll-anchor.mjs` prints the swap's offset delta and can no longer report `swap moved 0px` for a body whose growth never reached the document.** A finding reproduced 2 of 2 in CI and 0 of 3 locally (a full webkit sweep included), and the printed line — `floor alone: clamped 0px, reader 120px` — had no term that could tell "the engine declined to anchor" apart from "the mark misreported": both read as `clamped 0px`. `swap()` now also returns `after.pos - before.pos` for both the corrected and floor-alone passes, printed beside the field each disambiguates (`swap moved 0px [offset +120]`, `floor alone: … reader 120px [offset +120]` on a healthy cell, `webkit owrtsnap @1440 side normal`) — a 0 there is the engine not having engaged at all, where a healthy pass shows the whole pad's height arriving as compensation. Separately, and independent of that CI report: the body `SWAP` picks can sit inside `.fs-ovl`, the grid `fs-overview.js` wraps System/Memory/Storage in, whose row tracks size off the taller sibling column — so a shorter column can absorb the whole 120px pad without the row, and so the document, growing at all. Measured live, `div#fs-ovl-panel-0` at 291px on `owrtsnap @900` and again on `owrt2410 @1200` grew the document 0px and printed `moved 0px`, indistinguishable from a correction working; reproduced deterministically on a synthetic page (300px spanning column against two 280px siblings summing to more than its post-growth total) with the exact same shape. `swap()` now measures `document.scrollHeight` before and after the refill, and a cell where that delta comes back under half the inserted pad folds into `swap.skip` — the branch "nothing above the reader big enough to collapse" already uses, so it is printed as a named skip rather than silently joining the pass line. The anchor node's own identity (which element `fs-fit.js` chose to hold the line) is not reachable the same way: it lives in a private `_rest.el` with no exported accessor, so adding it would need a change to `fs-fit.js` this task did not make.

- **`owlab.yaml` records that the snapshot box's ordinary package set can differ from the release stands', beside the package list rather than only in a comment nobody reads before trusting a snapshot-only finding — and, one task later, the specific name it first recorded was already stale, which is now the point the comment makes instead.** The same investigation above traced the `owrtsnap @1440 side compact` finding's real cause to a DOM difference, not a theme fault: the Overview page rendered 12 `.cbi-section` on `owrtsnap` against 13 on `owrt2512`, `luci-app-https-dns-proxy` absent that day. `apk info -R` shows neither it nor its `https-dns-proxy` binary needs a kernel module (`ca-bundle libc libcares libcurl4 libev jsonfilter resolveip` only), ruling out the already-documented kmod-tun/kmods-index gap; `/var/log/apk.log` from the box's own most recent boot instead showed the KMOD-needing packages (`luci-app-mwan3`, `-sqm`, `-nlbwmon`, `-openvpn`, `luci-proto-wireguard`/`-openconnect`) failing with that gap's exact error while `luci-app-https-dns-proxy` installed clean in the same run. `feeds/luci` and `feeds/packages` build the snapshot index independently, so a `luci-app-*`/binary pair that agrees when both are fresh can briefly disagree while one side has moved and the other has not mirrored yet — not reproducible on demand, and not fixable with a version pin: `packages:` carries no version syntax, and a hard pin would go stale as the snapshot feed prunes old builds where a release branch would not. By the next task, `https-dns-proxy` installed clean on every stand and `luci-app-mwan3` was the one missing instead (no `90_mwan3.js`; the same `unable to select packages` line in `apk.log`, this time for `mwan3`, `nlbwmon`, `openvpn` and `sqm` together) — two different names in two sessions is the finding, so `owlab.yaml` and `docs/development.md` no longer name either as if it were fixed: both now say to check what is missing TODAY (`apk info -e <pkg>` / `grep 'unable to select' /var/log/apk.log` on `owrtsnap`) before treating a snapshot-only section-count or DOM-count difference as a theme finding.

- **`owrtsnap` rejoins `live`/`anchors`, and the `live-snapshot` job (0.14.12, above) is gone: owlab's own fix removes the reason it existed.** owfeed/owlab#18, filed this morning against the exact fault the previous entry contained, closed within the hour: `owlab` 0.6.1's `internal/pkgmgr.UpdateShell` no longer opens `apk update`/`opkg update` under a bare `set -eu` — the refresh now continues once at least one feed answered, and only aborts when none did, so a stale snapshot rootfs's dead kmods index costs one dead feed rather than the whole boot. Reproduced locally before restoring anything (`owlab up owrtsnap`, `owlab install owrtsnap dist/noarch/luci-theme-footstrap-*.apk`, both under a freshly downloaded `owlab 0.6.1 (3f20d9adfe57, 2026-09-07)` binary — the WSL install had drifted to a local dev build, `dev (f2da29f01c24-dirty)`, no good for proving a release fix): the same 404 the containment commit measured still prints (`ERROR: wget: exited with error 8`, `unexpected end of file` on `kmods/6.18.33-…/packages.adb`), but the router now boots and installs anyway, and the refresh names its own gap — `owlab: apk update: partial refresh, 7 feed(s) read, 10206 packages available; the feed above did not answer and its packages will be missing`. `.github/workflows/build.yml` bumps every `owfeed/owlab/setup`/`owfeed/owlab/action` reference (five call sites) to v0.6.1, drops `live-snapshot` outright, and restores `live`/`anchors` to the three-router `owrt2512 owrt2410 owrtsnap` set on a push. `owlab.yaml`'s `25.12.4` pin is unchanged: v0.6.0 moved owlab's OWN internal test matrix to 25.12.5 once `openwrt/rootfs:x86_64-25.12.5` existed on Docker Hub, but that tag still does not exist (`docker manifest inspect` against it 404s today) and the fallback-to-tarball behaviour the same release notes describe belongs to owlab's own `images/owlab.yaml`/`owlab context`, not to a consumer router's `release:` field — nothing here changed what tag this project's stand resolves. `docs/ci.md`'s job graph, job table and routers paragraph revert with it; `docs/development.md`'s trap note is rewritten from "how to tell an infra-red boot apart from a real regression" to what a partial refresh now looks like, since a dead feed no longer reds the boot step at all.

- **`owlab.yaml` stops asking the snapshot box to run three packages it can never resolve, one layer past the `owlab` 0.6.1 fix above.** That fix stopped a dead feed from cancelling the whole boot; it did not change that `defaults.extra_packages` reaches every router, snapshot included, and `owlab`'s own merge has no subtraction syntax (`internal/config/config.go`: additive only, default plus what a router adds) — so a package one router can never have still fails `owlab up` for the whole lab, by design (`reportMissingExtras`). Reproduced `owlab up --rebuild owrtsnap`: openclash and ssclash need `kmod-tun`, which resolves through a kmods index keyed to the box's exact kernel git hash, and `downloads.openwrt.org/snapshots` was not publishing kmods for the box's baked `6.18.33` hash (`apk add` 404s that index); justclash carries no kernel module and installed cleanly on the same box, which is why it was never in the failure list. `openclash`/`justclash`/`ssclash` now live on `owrt2512` (anchored, aliased onto `owrt2410`/`imm2512`/`imm2410`) instead of `defaults:`, so `owrtsnap` gets none. Verified on the stand: `owlab up owrtsnap` exits clean with no missing-extras warning, all 35 `*.ut` compile under its real `ucode -T -c`, `/luci-static/footstrap/cascade.css` and both admin pages answer 200 after login; `owlab exec owrt2512 -- apk list -I` shows all three apps `[installed]`.

- **`install.sh` no longer dies the moment one apk/opkg feed is unreachable — it continues once at least one feed answered, unless the ONE that failed is this project's own, in which case it still fails closed.** Same dead-kmods-index shape as the two entries above, one layer further down, inside the installer's own `set -e` with no per-package loop after it to absorb the exit: `install-check` (run 34112646188) died at `apk update` on `owrtsnap` before the theme was ever fetched. "Packages available > 0" is NOT the signal — with every feed unreachable apk still prints `8 unavailable, 0 stale; 136 distinct packages available` and exits non-zero, and those 136 are rows the router already had installed, not anything the refresh just read. `feed_refresh()` reads the `N unavailable` count against how many feeds were configured (`/etc/apk/repositories` + `repositories.d/*.list`): `N < configured` tolerates and prints which feed did not answer, `N == configured` fails closed exactly as before. opkg prints no such summary line and is affected identically (one bad feed among several still exits non-zero), so the same comparison runs there against `Failed to download` lines and the configured `distfeeds.conf`/`customfeeds.conf` entries. A follow-up security review (task 0176) found that tolerance too even-handed: `repo.owfeed.org` is a distinct host from every stock feed, so an on-path/DNS attacker can blackhole it alone while the rest answer, and the router would go on installing from whatever owfeed-packages index it had cached, reporting success on a stale or missing package. `feed_refresh()` now checks the failure lines for `$FEED_HOST` — the literal string this script itself writes into the repository entry, present in full in both managers' real failure output (apk's `ERROR:`/`WARNING:` lines, opkg's `Failed to download <url>`) — before tolerating anything, and fails closed with a message naming this project's own feed specifically when it is the one down, no matter how many stock feeds answered. Verified on four live stands: `owrtsnap` (apk, the real dead kmods sub-index, an unrelated host) still installs — `1 of 9 configured feed(s)` reported and skipped; `owrt2512` (apk) and `owrt2410` (opkg) with `repo.owfeed.org` blocked and every stock feed open now fail closed, installing nothing, naming `repo.owfeed.org` specifically rather than reporting success on a stale cache; both routers with every feed healthy install clean with no warning; and `owrt2512` disconnected from all networking still fails and installs nothing, naming `Could not read the apk feed index` and `Could not resolve host: github.com`.

- **A tap on a tab no longer defers the chrome fit for 400ms: tab pills stop resizing a beat after every tab switch on iOS.** `touchstart` sat in `fs-fit.js`'s `noteUser` list alongside `wheel`/`touchmove`, so ANY tap — a stationary tap on a tab included — called `noteMotion()` and declared the page moving for `SCROLL_IDLE` (400ms); `fitChrome()` defers `fitTabStrips()` for as long as `scrolling()` says yes, so a freshly drawn strip painted at full padding and only shrank once the motion sampler saw the page still, ~400ms after the tap. `touchstart` moves to the `noteIntent` list instead — it says the reader is present, not that the page is moving, and still feeds `_userUntil`, so `lateDrift()`'s "the reader is driving" gate is unaffected. Real motion is still read from the scroll position by `sampleMotion`: `touchmove`, `scroll` and momentum all still start it, so nothing that actually moves the page loses its guard.

- **The Overview stops oscillating once per poll tick on WebKit — the engine's own scroll anchoring is now turned off where it mis-fires, instead of racing the theme's correction for it.** `CSS.supports('overflow-anchor', 'auto')` used to be enough to tell "the engine anchors" from "it does not", but WebKit 26 shipped the property too, so every engine now answers `true`. Parked mid-page with real poll ticks landing (`tools/scroll-anchor.mjs`'s new `tick` case, below), WebKit's own anchoring still moved the offset — no `scrollTo`, no `scrollTop` setter recorded — on ticks that grew nothing above the reader by even a pixel: 21px on the Overview's default park (`../tmp/task-overview12/tick-probe.mjs`), 41px at 390 wide in the bar/top layout. `lateDrift()` (`fs-fit.js`) already wrote the correction back, one rAF plus `SCROLL_IDLE` later — measured 421/421/408ms after the tick — which is not wrong, just late enough to read as a jump rather than as nothing happening. `ENGINE_MISANCHORS` (`fs-fit.js`) is the new seam: `-webkit-hyphenate-limit-before`, a non-standard WebKit hyphenation extension Blink and Gecko have never implemented, answers `false` on Chromium and Firefox and `true` on WebKit — `-webkit-touch-callout` was tried first and rejected, since it answers `false` on a touch-less desktop WebKit too and so names a capability rather than the engine. Where it answers `true`, `fs-fit.js` writes `data-fs-anchor-suppress` on `:root` once, at module eval, and `styles/theme/20-shell.css` turns `overflow-anchor: none` on `html`/`body`/`#maincontent`/`.fs-main`/`#view`/`#view *` under that same attribute — the exact set `tools/scroll-anchor.mjs` already forces off to test the theme's own correction in isolation — so the misbehaving engine stops anchoring at all and the theme's already-measured, already-0px non-engine path (`scheduleAnchor()`/`applyAnchor()`) runs there instead. Chromium and Firefox never get the attribute and keep their own anchoring exactly as before: measured 0px before this change and 0px after, on the same cell. `tools/scroll-anchor.mjs` gains the `tick` case this fault needed: `held`/`swapped` insert a synthetic pad and close within 800ms, and `quiet` discards any step where the offset held still for 400ms — both windows a 421ms-late correction lands outside of, which is why a real regression here read as "the reader stayed put" on every previous case. `tick` parks the reader, leaves the poll running, and watches the offset across real ticks timed off the page's own `L.env.pollinterval` rather than a fixed guess. Because the fault is layout-dependent in a way the gate's other three cases are not — 0px at `390 side` (the sidebar collapsed to a bar) against 41px at `390 top` (the bar layout proper), same page, same tick, same engine, something `data-layout` still carries under the collapse — `390 top` joins the default (non-`--full`) axis for every case rather than only under `--full`, so a regression here is caught on a pull request rather than only on a push or a tag.

- **The Overview no longer creeps once per poll tick on ANY engine, and the WebKit-only fix from the entry above is gone — its diagnosis was wrong.** `ENGINE_MISANCHORS` blamed WebKit's own scroll anchoring for the drift measured there; the real mover was `fitChrome()` (fs-chrome.js) pinning the bar's box against SHRINKING while it measures whether the menu still fits, but never against GROWING — with the layout classes off, and again as `fs-bar-stack`/`fs-ind-compact` are added back one at a time, the bar's own height was free to answer whatever its content needed at that instant. `tools/fit-quiet.mjs` used to watch only the dip; extended to watch both directions, it measured the walk directly on the unfixed code: 230 -> 202 -> 164 -> 144 -> 123 -> 131 -> 123px against a settled 123px — 107px of growth a floor-only gate never saw. The Overview's own poll batches System/Memory/Storage into several separate `MutationObserver` callbacks rather than one, so `fitChrome()` runs, and paints, more than once per tick, and every engine followed that walk, not only WebKit: Chromium and Firefox hid it behind their own scroll anchoring, correcting each step before the next one landed, while WebKit has none of its own to hide behind and showed the drift directly. `fitChrome()` now pins the bar's `min-height` AND `height` to the same value for the whole decision and releases both only once the final class set is chosen, right before `publishBarHeight()` reads it — nothing the pass measures (`stripFitsOneRow()`'s `offsetTop`, `clusterFitsBrandRow()`'s widths) depends on the bar's own height, so the pin changes no decision, only what the reader would have been shown while it was made. `fit-quiet` now reads 0px peak-to-peak on chromium and webkit, all three stands, every width. `ENGINE_MISANCHORS` (`fs-fit.js`) and the `overflow-anchor: none` rule it drove (`styles/theme/20-shell.css`) are removed: with the bar fixed, the same probe that measured 21-41px on WebKit reads 0px at 390/top with NO suppression at all, 26s of real poll ticks (`../tmp/task-toplayout/top-probe.mjs --unsuppress`) — there was never a WebKit-specific fault for it to hold. `tools/size-budget.mjs`'s `coldJs` limit comes back down from 58,250 B to 58,200 B: removing the wkanchor mechanism returns 162 B on its own, the bar's two-way pin costs 61 B back, 101 B down net. `docs/anchoring.md`'s account of task wkanchor is corrected in place rather than left blaming WebKit. Verified the `tick` case (`tools/scroll-anchor.mjs`) at 390/top reads 0px on webkit, firefox and chromium, on both `owrt2512` and `owrt2410`.

- **The outgoing page keeps its own page-scoped styling and scroll position for the whole staging window now, not just until the click.** `fs-router.js` stamped `body[data-page]` with the INCOMING page's name at the START of every client navigation, but `body` is the ancestor of both pages at once while one renders hidden in the stage — so every `body[data-page="<outgoing>"]` rule in `styles/pages/*.css` stopped matching the page still on screen for the whole require. Measured, Overview → another page: the attribute flipped at 19ms, the swap landed at 1,426ms — 1,407ms with 33 rules not applying (port icons, `.fs-ovl-empty`, the stray `h2[name="content"]`, the progressbar tables), the document growing 211px and the reader moved 140px under them with no navigation of their own; isolated with no navigation at all, flipping the attribute alone measured the same +211px on both `owrt2512` and `owrt2410`, and package-manager (the other page with its own `styles/pages/*` file) measured -18px. `styles/pages/20-overview.css`, `30-software.css` and `40-sshkeys.css` now key their page-scoped rules off `#view[data-page]` (content inside `#view`) and `.fs-content[data-page]` (the dispatcher's own stray heading, a sibling of `#view` rather than a descendant) instead of `body[data-page]` — the same two-phase shape `fs-sheets.js`'s `scopeToCurrentPage(rsegs, leaving)` already uses for a page's own injected stylesheets: the incoming name lands on the hidden stage's `#view` as soon as it is created, so the staged render still measures itself under its own rules, while the LIVE elements keep the outgoing name until `commitStage()` moves it forward. `body[data-page]` itself is untouched — fs-chrome/fs-fit/fs-overview/menu-footstrap-common still read it at the same point, as the route's own identity, never as a CSS scope. The reader's scroll position moved off the click too: `window.scrollTo(0, 0)` used to run at the click, throwing the reader to the top of the OUTGOING page for the whole window (`y` reached 0 within 12ms and stayed there for 1.2s with the incoming module's fetch held open); it now runs in the same synchronous turn as the content swap, so the reader keeps their own offset until the new page's content actually replaces what they were reading. `tools/spa-parity.mjs` gains `stagingWindowCheck()`, sampling the outgoing page's document height and one of its page-scoped rules mid-flight rather than only after arrival, which is how this went unmeasured in the first place; it read 3584 → 3795px and a hidden rule turning visible before this fix, 0px movement and the rule holding after. `styles/theme/90-responsive.css` carried the same bug one layer over and went unmeasured for the same reason twice: ~11 package-manager rules scoped through `body[data-page="admin-system-package-manager"]` inside a narrow-viewport `@media (max-width: 767px)`, invisible to every case above because they all sampled at the 1440px desktop context. `stagingWindowCheck()` now also runs a narrow-viewport pass, its own browser context at the width read out of `90-responsive.css`'s own `@media` rule (767px) rather than a guessed constant, so an edit to the breakpoint cannot make the pass silently stop testing anything; on `owrt2512`, before this fix, it read `#view .controls > div:not(.pager)` as `flex` mid-flight where the stacked layout wants `block`, with the document height moving 22340 → 22172px, and 0px movement with the rule holding `block` after the same re-scope to `#view[data-page="admin-system-package-manager"]`. `docs/spa-router.md` ("The staging window") and `docs/anchoring.md` ("The scroll reset") carry the rule and the numbers.

- **A push whose feed assertion never ran no longer reads like one where it passed.** `verify`'s "Assert the feed serves a working theme" is gated on `tools/feed-key.sh` reaching `repo.owfeed.org`, and when the feed did not answer the step's `if` went false: the whole trace of a missing assertion was one `::warning::` line inside another step's log, and the job was green — the one thing a green run must never be able to mean. `tools/feed-key.sh report` now runs on every push whatever happened, and writes one of three verdicts — measured and passed, measured and failed, NOT MEASURED — into the run's annotations and job summary. The skip stays non-blocking: it tests an already-published channel this repository does not own, not this build. `tools/ci-local.sh verify` prints the same sentence from the same script.

- **A section a poll refills twice in a row no longer leaves the reader 47-64px down the page.** `holdFloor()` clears every floor before it re-measures — that clear is what makes the answers honest — and for the length of the measure pass the document stands without them, one pixel short of what the reader's offset needs. Measured live (chromium/`owrt2512b` `@390 top normal`, `/admin/network/dhcp`): 38 sweeps in one run, 33 of them took the document down between the clear and the write-back, 13 took the offset with it, and on 8 the document came back and the offset did not — 4730px → 4729px → 4730px against an offset of 3886 → 3885 → 3885, with 0 sweeps moving it the other way. One pixel is enough: `lateDrift()` reads the offset twice `SCROLL_IDLE` apart and treats any difference as the reader having moved, so the same tick's 60px correction was thrown away as "the reader is moving", and the unforced `rememberRest()` a millisecond later adopted the wrong offset as the reference the NEXT refill measures against — 59px, carried forward, which is the drift the reader actually met. `holdFloor()` now reads the offset and the scroller's height before the clear and, where the offset ended lower AND the scroller is as tall again as it was, writes the pre-sweep offset back through `writeOffset()` — the same door both corrections use, so the motion sampler reads the restore as this file's own rather than as the reader arriving. The height test is what keeps a genuine content shrink's clamp — which belongs to the shrink, and which `lateDrift()`'s `floorShrink` path already corrects — out of it: restoring unconditionally is 70 B cheaper and green on the same cell, but it makes every real clamp this file's own write and closes the motion window `sampleMotion()`'s terminal sweep runs behind. The cell reads `3x repeat 0px/-47px/-47px` → `0px/0px/0px` through the real gate, its floor-only ablation `clamped -59px, reader -47px` → `clamped 0px, reader 1px`, and a default-axis sweep across `owrt2512b`, `owrt2410b` and `owrtsnapb` on all three engines now carries no `REPEAT` position finding anywhere. `tools/floor-contract.mjs` is unmoved (62 floors, worst -1px). +106 B minified, which is what raises the `coldJs` and `resourcesJs` budgets in `tools/size-budget.mjs`. `docs/anchoring.md`, "The sweep's own clamp".

- **A shrink above the reader no longer leaves them 60px down the page on WebKit's inner scroller.** When content above the fold shrinks, the browser clamps the offset only by the height the document lost *below* it — measured on `webkit`/`owrtsnapb` `@1440 side compact`, Overview: the scroller went 2793px → 2673px and the offset was clamped 1889 → 1829 where 1769 was what the reader's own mark needed. `applyAnchor()` is the only thing that returns that remaining 60px, and it refused — on `scrolling()`, which the clamp's own scroll event had just turned on (`apply-enter scrolling true, moving 400ms, at 1829`, the correction already computed at `drift -60` and thrown away). `rememberRest()` a moment later then took its reference at the clamped offset (`_rest.top` −362.625 → −422.625), so every later refill honestly measured 0px of drift against ground that was already wrong, which is the `corrected never` the gate reports on refill 2 and 3. `holdFloor()`'s real-shrink branch — the one that deliberately lets a genuine clamp stand, because that clamp belongs to the shrink — now records the offset it left behind (`_clampedTo`), and `applyAnchor()` reads it through `sawClamp()`: `scrolling()` still refuses a reader who is moving, but not when the only motion on record is a clamp this file just watched happen. The cell reads `3x repeat 0px/-60px/-60px, corrected never` → `0px/0px/0px, corrected 34/23/23ms`, and the next pad removal stops clamping at all (`at 1769, landed 1769`). Measured against the regression this risks — a correction landing inside a reader's flick — on the full axis across three stands and all three engines: `mid-flick surprises 0` on all 552 cells, 2 findings against the 3 the same axis carried before, both of them present before this change too. +53 B minified, inside budget. `docs/anchoring.md`, "A clamp is not the reader".

- **The floor the theme writes is no longer half a pixel taller than the content it was measured from — which is what made the browser clamp the reader and the engine drop its anchor.** `holdFloor()` took each box's height from `offsetHeight`, an INTEGER rounded to nearest: 421.875 written back as 422, 40.75 as 41, 292.719 as 293, 1685.656 as 1686. Twenty-two such boxes on one page made the document 2px taller WITH the floors than without, so every clear at the top of the next sweep gave those 2px back — 7422 to 7420 on 57 of 64 sweeps — and a reader parked at the end of the document had their offset clamped into the gap. A clamp is a scroll position change, and a scroll position change invalidates the engine's own anchor node (css-scroll-anchoring-1 §2.1.1), so the growth that arrived next was left uncorrected. That is the whole of the 47-64px `REPEAT` residual this release has been chasing through four separate mechanisms. Measured by separating the four events between two refills, 12 cycles each with the clamp forced so nothing else varies (`owrt2410b`/webkit `@390 top normal`, Overview): no clamp 12/12 anchored, a clamp with the theme off 12/12, the sweep and the clamp together **6/12**, and the floor written from `getBoundingClientRect().height` **12/12**. The same separation clears both of this release's own recent corrections of the suspicion: removing `holdFloor()`'s 1px restore reads 6/12, removing it together with `applyAnchor()`'s clamp write reads 6/12 — neither of them is what the engine declines for. The two cells CI still failed on read `_engineTrusted went false` + `corrected 428ms` to `3x repeat 0px/0px/0px, trusted true->true, corrected 30ms`, and `refill 2/3 -49px corrected never` to `0px/0px/0px, corrected 16ms`. `tools/floor-contract.mjs` reads 175 floors at worst -1px to 0px. Full axis, three stands, all three engines: 184 runs each, no findings, `mid-flick surprises 0` on all 736. Also measured: **14 cycles is the minimum that separates a 0.8 success rate from a 1.0** — an earlier 13-of-14 ablation that pointed at the floor WRITE never reproduced because 14 was where it stopped, and `docs/anchoring.md` keeps that disproof beside the finding.

- **A page with a table no longer stops responding.** `/admin/system/filemanager` pinned the browser's main thread at ~105% CPU under `13e9864`, and the tab never came back — a bisect by swapping one file on a live stand put it exactly there: the stock bootstrap theme answers a probe in 4 ms, `fs-fit.js@76b3c7a` in 4 ms, `fs-fit.js@13e9864` never. That commit widened `_moTabs` to `hidden`, `aria-expanded` and `class` so that content hidden IN PLACE gives its floor back, and it treated every attribute record as a change. It is not: a same-value `classList.add()` still queues a mutation record, and the fitters inside `run()` re-add `.th` to already-marked cells on every pass. Instrumented with a wrapper over every `MutationObserver` on the page (`owrt2512b`): **391 callbacks in 432 ms — 926 a second, stopped only by the probe's own budget — 3910 records, 100% of them `class`, all written by the previous callback's own `tagDataTables`/`adoptMarkup`/`fitTables`, 2340 of them on the same six `th[data-field]`**; the `childList` observer took 2 callbacks in the same window, so this was a self-feeding loop and not the page being large. `_moTabs` now registers with `attributeOldValue: true` and drops records whose value did not actually move: 391 callbacks/3910 records/240 ms become **3 callbacks/184 records/1 ms**, the probe answers in 1-3 ms on all three stands, and a settled page churns 0 records per second. What `13e9864` bought is intact and re-measured: the Appearance disclosure goes 731 / 1485 / **731px** with `HELD=0px` (754px of dead ground before it), and stock LuCI's `depends()` row on Time Synchronization takes its section floor to **49.5px** against 50px of bare content, also `HELD=0px` (258px before). `tools/floor-contract.mjs` gains the case that catches this class — liveness asked BEFORE anything else, with a deadline of its own, because `page.evaluate()` has none and a frozen page used to read as "no floor standing" and be skipped silently — and it was seen red on the pre-fix build deployed to a stand (`the page's main thread did not answer for 10s`, exit 1) before the fix was accepted.

- **The live job's `parity` and `audit` slices finish again, and `motion` went from 39 minutes to 13.** Both slices were cancelled at `timeout-minutes: 45` on four consecutive runs with nothing in either log, so two live gates measured nothing for a day while the run reported only the third. Two causes, separately measured. The first is below — one page stopped answering and `classify()` has no deadline. The second is that `motion` still carried chromium's own `scroll-anchor` sweep, which grew from 145s to **1434s** when `--full` started crossing the density axis (`e48434c`), putting the slice at 39 of its 45 minutes with the other six gates behind it; it moves to `anchors` as a third shard, where the cap is 75 minutes and the one-job-per-engine rule already lives, and the cells are the same cells. **`motion` 2188s to 754s**; `anchors` takes the 1434s beside its own 1722s and 1696s, so the pipeline's wall clock does not move. The slice comment in `.github/workflows/build.yml` is brought back to the measured numbers — 485s (spa-parity), 450s (live-audit), 754s (`motion`'s six), 157s of per-slice overhead of which 128s is `owlab up` plus install — from an estimate written when the gates did less. Both gates now print a line per page with the clock on it, so a slice that is killed names the page it was on instead of leaving an empty log, and `tools/ci-local.sh` carries the same layout rather than a second, drifting one.

- **A page that loads and then stops answering is a finding with a name on it, not a slice that dies in silence.** `page.evaluate()` has no deadline in Playwright, and `classify()` — which walks the whole menu and is called by both `spa-parity` and `live-audit` — sat on one forever: a page that pins the browser's main thread does not cost a page's worth of seconds, it costs the slice its entire budget. Each shape probe is now capped at `PROBE_DEADLINE_MS` (10s), about **2000x the 4-5ms a live page answers in**, and wide enough that even a menu where every path froze costs 16 minutes rather than all 45 — 96 paths is the widest measured (`owrt2512b` with the third-party fixtures). A page over the deadline is printed as a FINDING naming the router and the path, mid-walk; the walk continues on a **fresh tab**, because a frozen page stays poisoned — every later `goto` on it burns its own 20s — while `close()` plus `newPage()` costs 0.5s and the login lives on the context; and `parity` and `audit` exit non-zero instead of green. A page that never LOADED still says nothing, deliberately: that is the runner or the stand, not the theme, and the two must not read alike — the same split `tools/scroll-anchor.mjs` already makes. Seen red both ways before it was accepted: synthetically, a page with an infinite loop after `DOMContentLoaded` went from `classify()` never returning in 150s to a FINDING with 3 of 5 paths still classified and status 1; and on live stands, `/admin/system/filemanager` under the freeze fixed above gave `spa-parity --only owrt2512b` a FINDING at 35s with 44 of 46 pages still compared, exit 1.

- **Back no longer drops the reader 431px down a page that never moved.** `fs-router.js` did its part correctly — `commitStage()` calls `restoreScroll({win: 2723})`, the first tick reads a ceiling of 3152 and writes 2723 — and then, 429ms later (`SCROLL_IDLE` plus a tick), `lateDrift()` in `fs-fit.js` wrote 2292.21875 over it. The SPA commit reaches `observeContent()`'s observer in TWO batches: `run()` -> `rememberRest()` fires between them and takes its reference in the middle of the swap, and the second batch hands that half-swapped reference to `lateDrift()`, which reads the swap's own settling as the reader's drift. Three independent pointers named the writer before a line was changed: `fsAnchor='off'` restores 2723 on 3 of 3 runs; `fit.forgetRest()` at the commit still gives 431px, because the reference comes from the observer and not from `_rest`; and `fs-fit.js` taken from `50112c4^`, `71295ce^` and `28788d0^` gives 431px too, so none of this release's own anchoring work is involved. The page really was still: 400 nodes of `#view` re-read 60ms and 2.5s after the commit differ in 11, all of them zero-height `IMG`/`BR` inside `fixed`, whose document coordinate moved by exactly the scroll delta. The guard already in `lateDrift()` cannot see this — `navigate()` and `commitStage()` move every page stamp before the commit is observable, so the stamp reads `admin-status-overview` both at capture and at write, which is why putting the stamp on the reference itself was measured and dropped. It only ever showed on 24.10: there the incoming page is whole at swap (400 of 400 nodes, document at its final ceiling) so the UA restore survives and there is something for the correction to overwrite; on 25.12 and snapshot the commit lands at 61-66 nodes, the UA restore clamps to 0 and `restoreScroll()` writes from zero (2683 to 2683, 2473 to 2473). Also measured and NOT shipped: holding `restoreScroll()` for its whole 5s budget reads 2723 on 2 of 2, but it only undoes the correction a frame later — the reader still sees the jump — and it overrides the scroll-cancellation that three fixes in `64b183f` stand on. `tools/spa-parity.mjs` gains the lever that makes this reproducible: it holds the incoming page's ubus for 700ms across the traversal and waits on `.fs-staging` rather than on the clock, which takes the case from roughly one run in fifteen to **red on every run** against the old behaviour (`parked at 2723px, Back restored 2292px`). Verified after: 2723 to 2723 with `writeOffset()` not called at all, 3 of 3; `spa-parity` agrees on 46, 42 and 40 pages across the three stands; and the anchoring the late path exists for is untouched — `scroll-anchor --full` reads 184 runs and no findings on chromium, firefox and webkit alike, 552 in all.

- **A refill a trusted engine declines to anchor is corrected in 8-61ms instead of 419.** `lateDrift()` waited `SCROLL_IDLE` (400ms) past its own first frame before it would write, whatever the page had been doing, so the reader ended up in the right place but saw a jump getting there — which is exactly what the sweep's 200ms `LATE_MS` threshold exists to call out. It survived **three CI runs of one commit: green, red, red, the same cell each time** (`webkit owrtsnap @1440 side compact, engine-anchoring on, overview`), and a full local sweep of 552 runs found it zero times, because on an unloaded machine WebKit anchors that refill itself and the late path is never the corrector. The green run was the engine working, not the theme. The wait is there because one frame cannot tell a flick from a still page by the offset alone — a flick moves it in steps of tens of milliseconds, and 120ms was measured letting a 160px correction through on a loaded runner — but the file already answers the question from two sides: `scrolling()` is its own reading of "has anything moved the offset in the last `SCROLL_IDLE`", sampled from the position every frame, and `_userUntil` is the reader's hand on the page, since `touchstart`, `wheel`, `mousedown` and `keydown` all arrive BEFORE the offset they are about to move. Where both say nobody is driving, the only thing that can have touched the offset since the reference was taken is the engine, whose own correction window measures 7-36ms. So the wait forks — the next frame where the page was already still, the full `SCROLL_IDLE` where it was not — and the `scrollTop() !== seen` check that decides WHETHER to write is unchanged and runs on both paths. Two negative results are kept in `docs/anchoring.md` rather than discarded: an intent-only fork (the direction this started from) would take the short path straight through `QUIET`, whose 24 flick steps are driven by assigning `scrollTop` and so carry no intent event at all, and would miss iOS momentum, which outlives the last event by design; and a third frame costs 11-16ms of the margin under `LATE_MS` for no change in any reading (webkit 67/71/72ms against 51/54/61ms). **`tools/scroll-anchor.mjs` gains the axis state that could not be reached before.** `noEngineAnchor` had two: with the engine on the engine does the work, and with `fsEngineAnchor=off` the theme KNOWS it is alone and takes `applyAnchor()` (7-36ms). The state CI caught is the third — the engine declining while the theme still trusts it — and it is one `addInitScript` away, the same `overflow-anchor: none` stylesheet without the `localStorage` half. It runs as a third cell per group, printed `engine DECLINES`, and was **red on the old behaviour 9 cells of 9 on an idle machine** (chromium 409/419/420ms, firefox 404/415/417ms, webkit 421/421/422ms) before the fix was accepted. Verified after: 828 runs across three engines and three OpenWrt lines, no findings, `mid-flick surprises 0` everywhere, and `spa-parity` agreeing on 46, 42 and 40 pages.

- **Six more stands, so a full sweep runs as nine shards instead of three engines in a queue: 75 minutes to 31.** `owlab.yaml` gains `-c` and `-d` copies of the same three OpenWrt lines — same releases, same third-party fixtures, so a finding on one carries the weight of a finding on the original. `-b` answered contention between two people; these answer contention inside ONE sweep. Running all three engines in a single process takes chromium's zygote down about two minutes in (`docs/development.md`), which is why the sweep had been run one engine at a time, ~25 minutes each, on a 20-core machine; one process per engine, each with its own router set, completes with no zygote loss at all. Measured: **1860 s for 828 runs** against ~75 minutes serial. The same run also says what NOT to do next: nine shards read 26 cells/min against three shards' 22 — 18% for three times the processes, because `tools/scroll-anchor.mjs` already runs its stands concurrently inside one process (0fa9c2b) — while `load average` sat at 1.49 of 20 and every stand container at 0.03-0.14% CPU. The sweep is wait-bound, not CPU-bound; the ~20 s a cell is SWAP's 900 ms window, three REPEAT refills, QUIET's 24 flick steps and `SCROLL_IDLE` on each check, every one of them a measured number that half of this week's findings need at its current width.

- **The growth witness climbs to the floored box, so a refill one level in is no longer read as no growth at all.** `observeContent()` matched a mutation record only when `m.target` ITSELF carried `data-fs-floor`, and `dom.content()` refills the node it is handed — which is often a level or two inside the box `holdFloor()` pinned. Measured live on Overview (`webkit`/`owrtsnapb`): of twelve nodes a poll refills on that page, one sits inside a floored box without wearing the mark and two have no floored ancestor at all. For those `grew` read 0, and where `lateDrift()`'s element-based `drift` was ALSO blind — `anchorRef()` having hit-tested something the growth never reached — the two together said there was nothing to correct and the theme wrote **nothing**: `writes: []`, `corrected never`, the shape CI reported on `webkit owrtsnap @1440 side compact overview` in three runs of three while every local run of the same cell passed. `closest(FLOORED)` is strictly wider than what it replaces — a target already wearing the mark is its own `closest()` — so no tick that used to find a witness can stop finding one. Two dependent reads move with it: `_deferredFloor` and `floorShrink` both took `r.target.style.minHeight`, which on an inner node is unset, so left behind they would have read a shrink the size of the whole box. Verified: 829 runs across chromium, firefox and webkit on three stands, no findings, `spa-parity` unchanged, +14 B on flash and +7 B cold.

- **A "never came back" finding now says what the theme's growth witness could see, instead of leaving that to be guessed.** `tools/scroll-anchor.mjs`'s SWAP reads, before the refill, whether the section it is about to empty has a floored ancestor at all, and prints it as `growth witness: none | self | div#id@<pinned height>` on both the never-came-back and the corrected-late findings. Three CI runs of one finding had already been spent guessing whether the theme attempted a correction and missed, or never had a number to correct with; the first run carrying this printed `growth witness: self@639` on two cells and settled that question in one reading. A second reading rides beside it — `theme still at refill: true|false`, read from `fs-fit`'s own exported `scrolling()` immediately before the refill — because `anchorRef()` returns null outright while the page is moving and `rememberRest()` then clears `_rest` while STILL setting `_restAt`, so the sweep's own wait (`restAt() === scrollTop`) is satisfied by a theme that holds no reference at all; `lateDrift()`'s first line is `if (_lateFrame || !ref) return`, which reads as `writes: []` and is indistinguishable in a report from a correction that ran and missed. It costs nothing on a router — the gate reads the DOM it is already driving, and no byte of it ships.

- **A late correction that does not write now says which of its eight exits it took.** `lateDrift()` can return without writing for eight different reasons and from outside they are one symptom, `writes: []`; six CI runs were spent guessing between them and each guess cost a push. `_lateWhy` records the exit — `busy`, `no-reference`, `anchoring-off`, `reader-intent`, `page-changed`, `offset-moved`, `reference-gone`, `engine-partly-<n>-of-<m>`, `no-drift-grow-<n>`, `drift-too-big`, `wrote-<n>` — and `lateWhy()` exports it for the sweep, unmarked, the same shape and the same reason as `restAt()` and `engineTrusted()` beside it. `tools/scroll-anchor.mjs` prints it on both timing findings alongside two readings taken at the refill itself: `growth witness`, which says whether the theme had a floored box to measure the growth against, and `theme still at refill`, which says whether its own motion sampler was quiet. The first run carrying all three settled in one reading what three runs of guesswork had not: on `webkit owrt2512 @390 side large overview` the theme reported `growth witness: self@2546, theme still at refill: true, theme said: wrote-120` at 219ms — the reference was there, the page was still, the value written was exactly right, and the only fault left is that it landed 19ms over the gate's 200ms threshold. Costs 254 B on flash and 247 B cold, which is cheaper than one more run of guessing, and comes out again once the timing is closed.

- **The late correction is no longer armed on the half of a refill that only takes nodes away.** `dom.content()` empties and then fills, and where those two halves reach `observeContent()` as two batches, `lateDrift()` armed on the first — aiming a correction at a page about to stop existing. CI measured what that costs, in the theme's own words now that it names its exits: `theme said: wrote--834` on `webkit owrtsnap @1440 side compact overview`, minus 834 pixels written for a refill that grew the page by 120, the reader thrown by the transient and nothing left for the real half — which from outside reads as `writes: []` inside the measuring window, with the damage done just before it. **The obvious repair was measured first and is worse**: cancelling the armed call so the later batch wins turned a clean sweep into 12 findings of ordinary drift — `engine-anchoring on`, Overview, the reader 46, 88 and 138px off across real poll ticks at every density and both layouts on two stands — because a real tick delivers around twenty records and each one cancelled the last, so nothing ever fired. Reverted, and kept in `docs/anchoring.md` as the negative result it is. What ships instead is the discriminator that survives both cases: a batch that removed nodes, added none, and left the floored box no taller is a removal, and a removal is not a page to correct against. A synchronous `dom.content()` — every real poll tick — delivers both halves in one batch and is untouched; 21 B on flash, 24 B cold. **The first cut of that guard shipped too wide and CI caught it**: skipping every pure removal also skipped the removal of REPEAT's pad between refills, whose `floorShrink` is carried into `lateDrift()` precisely so the clamp that follows is put back — `refill 2/3 left the reader -60px off, corrected never` on chromium and firefox, `/admin/network/dhcp @390`, both stands, on cbcfd5d, having been checked locally on webkit alone before the push. The guard now also requires the floored box not to have shrunk: on the empty half of a refill the floor holds the box and `floorShrink` is 0, on a real shrink it is not.
...
- **Content that arrives below the reader no longer moves them — a regression this release introduced and caught.** An earlier change in this release made the growth witness in `observeContent()` climb to the floored box a refill sits in (`closest()`), so a refill one level inside a box finally counted as growth — but nothing asked WHERE that box was. For a box that grew below the reader, the reader's reference correctly did not move, and `lateDrift()`'s blind-witness branch read that as a blind witness and wrote the whole growth. Measured deterministically with a probe that grows a floored box below the viewport by 120 px: the offset moved +120 and the reader -120 on firefox and chromium, `/admin/network/dhcp` and Overview, with the engine's own anchoring ON — an ordinary browser — and in the DECLINES state, 8 of 12 cells; the call stack named `settle()` in `lateDrift()`. The same probe on the fs-fit.js from before that change moved nothing in all 12. The witness now counts growth only from a box that starts above the reader's reference: a box that begins at or below it cannot move it. The probe then reads 0 px in all 12 cells. The sweep could never see this — every case grew content above the reader — and gains BELOW for it (next entry). `lateDrift()`'s decision trail also gains four markers (`frame`, `wait-frame`, `wait-idle-moving`/`wait-idle-intent`, `settle`); they are what showed the late corrections on firefox and webkit waiting for a frame the page did not produce.

- **The sweep checks that content growing below the reader leaves them where they are, and prints the longest frame gap beside a late correction.** Every case in `tools/scroll-anchor.mjs` grew content ABOVE the reader — HOLD at the top of `#view`, SWAP and REPEAT a body entirely above the viewport — so a theme that moved the reader for growth under them passed 828 runs of it. BELOW parks a quarter of the way down, grows a floored box entirely below the viewport by 120 px from one level inside it (the shape a LuCI refill has), and fails if the reader's mark moves; each cell line carries `below Npx` or `-`, so a cell that measured nothing does not read as a clean one. Seen red before it was accepted: on the fs-fit.js that shipped the regression, 4 of 4 engine-on cells (firefox and chromium, dhcp and Overview) read `below -120px`; on the fix, 12 of 12 cells read 0. Timing findings also print `longest frame gap: N ms ending at +T ms`, from the `performance.now()` the frame loop already calls — no layout read, unlike a per-frame geometry timeline that was tried and dropped because it read layout inside the frames being measured. The gap is what tied the late corrections to a stalled page: firefox `wait-frame +21, settle +261` beside a 240 ms gap ending at +261, webkit `wait-frame +18, settle +181` beside 163 ms ending at +181.

- **The sweep reads a frame as it is painted, so a stalled runner no longer reads as a late correction.** SWAP's and REPEAT's frame loops read the reader's mark at the top of their rAF callback, which is queued before the theme's own, so a correction made in that frame was credited to the next one. CI caught what that costs: firefox, `/admin/network/dhcp @390 top normal`, engine DECLINES — `the correction landed 2702ms after the refill`, while the late trail beside it read `settle+21 wrote-120+21` and the longest frame gap was 2681 ms ending at +2702. The mark is now read from a ResizeObserver re-observing the root every frame, delivered after every rAF callback and before paint, with no DOM write; each cell line prints `painted N/M`. Seen red before it was accepted: an fs-fit.js forced onto the 400 ms wait read 13 late findings of 13 on each of chromium, firefox and webkit. REPEAT's finding also gains the growth witness, the late and anchor trails and the longest frame gap SWAP's already had — a chromium `refill 2/3 … -60px, corrected never` arrived without them and could not be reproduced in 432 runs of its cell.

- **A correction on a still page no longer waits for a frame the page may not produce.** On the still path `lateDrift()` took one frame after a refill and asked for a second to settle in, and right after a refill a page can produce no frame for hundreds of milliseconds: firefox, `/admin/network/dhcp @390 top compact` — `wait-frame +21, settle +261` beside a 240 ms frame gap; webkit, Overview @1440 side — `wait-frame +18, settle +181` beside 163 ms. The reader saw the uncorrected frame for that long. The callback it waited from is already a frame, and reading the offset at its top forces the layout the engine adjusts in, so it now settles there; a page in motion or under the reader's hand still waits the full 400 ms. On the same cells the correction now lands at 3-18 ms on all three engines. `docs/anchoring.md` also records the growth-below regression, this change, the sweep's frame-read fault and the chromium REPEAT finding that did not reproduce.

- **A page that stops being a LuCI page in the middle of a measurement counts as a cell that never opened, not as a finding.** CI failed a webkit sweep twice in its last 40 runs on `the measurement threw — page.evaluate: TypeError: undefined is not an object (evaluating 'window.L.require')`, both on owrt2410 and both in runs where the same engine also reported `page.goto: WebKit encountered an internal error` — the document the cell opened was gone. After a throw the sweep now asks the page whether LuCI is still there, with the page that cannot answer counted as gone; only then is the cell set aside with the unopened ones, still under the same tolerance, so a stand that keeps losing its pages fails the sweep as before. A probe on chromium, firefox and webkit read LuCI present on a stand page and absent on `about:blank` and on a closed tab. Seen red before it was accepted: the case the throw is a finding for — fs-fit.js loaded with `restAt()` stripped — still fails the sweep with `exports no restAt()`.

- **The staging-window check tells a page still loading apart from a page-scoped rule letting go.** `spa-parity` loads the Overview, waits 1400 ms and clicks, and on CI the Overview was regularly still rendering: `the outgoing page's document height moved during the staging window` read `2959 -> 3540px`, `2750 -> 3331px` and once `900 -> 4011px` — its own sections arriving. CPU throttled 8x does not reproduce it (the Overview is complete at 1400 ms in 15 of 15 loads); ubus responses held back 800 ms do (900 px there, 10 of 10). Waiting for the page to settle was tried twice and does not work: height and mutations alone read a page standing on its loading spinner as settled, and adding no-request-in-flight never settled on CI within 20 s, failing three runs of three — that change was reverted. The check now counts the nodes added to or removed from the outgoing page inside the window: the defect it exists for renames an attribute and moves no node, a page still loading does. A height change with content mutations is tried again, up to three times, and reported as measuring nothing only if every attempt is like that; the page-scoped rule check still fails on any attempt. Under ubus held back 0-600 ms, 13 of 30 loads read the height moving and 13 of those came with content mutations. Seen red before it was accepted: an fs-router.js that renames the live page at the click still fails it, with no node moved.

- **A mid-flick surprise from the sweep names who moved the page.** CI reported `the offset moved on its own 1 time(s) mid-flick (worst 562px)` on firefox, owrt2410, `/admin/network/dhcp @390 top large`, with the engine's own anchoring on — once in the last 60 failed runs, and with nothing to say whether the theme wrote the offset or the engine adjusted it. The same cell did not reproduce locally: 324 runs on the current fs-fit.js and 324 with the still-path correction put back to waiting one more frame, no surprise in either. The finding now carries its first surprise's step, whether a pad was inserted on it, the offset asked for and read, how much the document grew, the trust flag, the late and anchor trails relative to the step, and the theme's own scroll writes, so the next report says which of the two it was.
