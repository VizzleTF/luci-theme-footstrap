...
- **The staging-window check waits for the page being left to finish loading before it measures it.** `spa-parity` loaded the Overview, waited a flat 1400 ms and clicked, and on CI the Overview was still rendering: `the outgoing page's document height moved during the staging window` read `2959 -> 3540px` and `2750 -> 3331px` in three runs and `900 -> 4011px` once — the page's own sections arriving, not a page-scoped rule letting go. Measured without a click: CPU throttled 8x leaves the Overview complete at 1400 ms in 15 of 15 loads, while every ubus response held back 800 or 1500 ms leaves it at 900 px there in 10 of 10. The check now waits until, for a second running, no XHR or fetch is in flight, no LuCI loading spinner stands in `#view`, the document height holds and the content does not mutate (capped at 20 s; a page that never settles is reported as measuring nothing), and a height finding says how many content mutations landed inside the window. Height and mutations alone were tried first and are not enough: at ubus +600 ms the Overview stood on its spinner longer than a second, read as settled, and grew inside the window in 4 of 4 loads. With the click, under ubus held back 300-1200 ms, the old sampling read the height moving in 4 of 16 loads and the new one in 0. Seen red before it was accepted: an fs-router.js that hands the live page the incoming name at the click still fails the check.
